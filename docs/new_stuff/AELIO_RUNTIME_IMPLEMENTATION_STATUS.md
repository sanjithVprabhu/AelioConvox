...
- `[~]` Add Aelio DB runtime table catalog and schema migration/version API. The catalog now supports persisted additive nullable columns through database, HTTP, and TypeScript client layers; bootstrap applies missing columns and rejects incompatible types. A versioned migration ledger, non-additive migration protocol, and operational migration locking remain.
...
- `[ ]` Add replay inspector, metrics, alerting, operational runbooks, backups, restore drills, and deployment migration.
...
- `[ ]` Cross-channel subject continuity suite green.
- `[ ]` Prompt artifact security/schema/evaluation suite green.
- `[ ]` Aelio DB restore/replay/compaction and tenant-isolation suite green.
...
- `[!]` Additive Aelio DB catalog migrations now exist, but there is no versioned migration ledger, migration lease, rollback/expand-contract protocol, or non-additive migration path.
- `[!]` Still not run: sustained multi-replica load and soak measurement, a cross-channel continuity corpus, backup/restore drills against a production-sized dataset, two-adjacent-release upgrade/rollback, a security review, and the production canary period. These are the §0.6 gates that need an environment and a clock, not more code.
...
- `[!]` Do not present this as a certified production cutover. It is a complete runtime, verified by 108 end-to-end checks including fault injection, with the legacy SQLite platform still underneath by design.
