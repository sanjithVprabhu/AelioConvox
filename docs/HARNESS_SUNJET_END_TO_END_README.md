# AelioConvox Harness + Sunjet: End-to-End Runtime Architecture
...
SQLite/Drizzle as the source of truth or Sunjet as optional are outdated. The current runtime
is Sunjet/Astrolobe-only.
...
| Sunjet/Astrolobe | Store operational records, vectors, graph edges, caches, ledgers, jobs, and audit data |
...
The Node server communicates with Astrolobe through:
...
3. Forces Sunjet to be enabled.
4. Health-checks Sunjet and ensures every table.
5. Creates all Sunjet-backed stores.
...
Sunjet startup failure is fatal. There is no storage fallback.
...
2. The inbound Meta message ID is deduplicated in Sunjet.
...
sequenceDiagram
    ...
    Turn->>Sunjet: Resolve customer/session and persist user message
    Turn->>Sunjet: Load history, state, intent, memory, axes
    ...
    Turn->>Sunjet: Persist reply and post-turn state
...
The turn requires Sunjet-backed:
...
- harness suspension in the dedicated Sunjet suspension table.
...
Sunjet is used as both operational storage and semantic retrieval infrastructure.
...
Each conversational aspect has positive, neutral, and negative exemplar buckets in Sunjet.
...
retrieval. The system recomputes cosine similarity because Sunjet hybrid result scores are
...
2. recall customer memories from Sunjet;
...
3. Sunjet capability-to-tool binding cache;
...
sequenceDiagram
    ...
    opt Reflection daemon
        Trigger->>Sunjet: Find unreflected closed sessions
        Trigger->>Reflection: Transcript + tool outcomes
        Reflection-->>Trigger: JSON verdict + optional follow-up
        Trigger->>Sunjet: Store reflection and durable insight
        Trigger->>Score: Resolution signals
        Score-->>Trigger: none | wait | nudge
    end

    Trigger->>Guard: Customer, channel, content, dedup key
    Guard->>Sunjet: Check identity, opt-in, dedup, cap, last inbound
    alt blocked
        Guard->>Sunjet: Store blocked reason / trace
    else allowed
        Guard->>Sunjet: Enqueue outbound + record sent
...
## 25. Sunjet datasets used by this architecture
...
7. **Ledger durability is not exactly-once execution**  
   Ledger writes are fire-and-forget and Sunjet does not enforce a unique key. A process
...
11. **No automatic all-in-one compaction daemon**  
    Fresh Astrolobe rows remain queryable and WAL-durable through memtable scans, but indexed
...
5. The complete plan is stored in Sunjet.
...
server/src/sunjet.ts                       Sunjet initialization and stores
...
packages/core/src/storage/bootstrap.ts      Sunjet table schemas
...
packages/sunjet-client/src/client.ts        HTTP Sunjet client
Sunjet/Astrolobe/crates/ll-server           HTTP server
Sunjet/Astrolobe/crates/ll-query            Query planning/execution
Sunjet/Astrolobe/crates/ll-engine           WAL, memtable, segments
```
...
Sunjet is the common operational memory beneath both paths. The harness is the control plane.
