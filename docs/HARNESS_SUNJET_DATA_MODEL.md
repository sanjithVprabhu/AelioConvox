...
## 1. Sunjet is a multimodal row engine

Sunjet/Astrolobe is not three independent databases. It is one typed row engine whose columns
...
normally rely on Sunjet's `embed` value or server-side semantic clause.

---

## 2. Physical row identity versus logical identity

Every inserted Sunjet row receives a physical numeric:
...
Astrolobe uses one database-wide catalog and one global physical storage stream:
...
Sunjet can execute vector, text, graph, and scalar operators over memtable and segment
...
These limits should be enforced at the server boundary before treating Sunjet as a
multi-tenant service.

For memories, tools, archetypes, aspects, and response cache, AelioConvox commonly:

1. requests candidate row IDs from Sunjet;
...
### Question 1: What is Sunjet's actual isolation unit?
...
```text
option A: separate Sunjet deployment/database per tenant
...
### Question 2: Does Sunjet support filtered vector search natively?

**Answer: yes. Filters are part of the native query plan, but execution is adaptive: Sunjet
...
Astrolobe has native breadth-first graph traversal:
...
Astrolobe internally uses WAL transactions for each individual mutation:
...
If Astrolobe gains a batch transaction API, group only local invariants:
...
This is memory, not a Sunjet table.
...
The active axis traversal does not use Sunjet's graph query clause.
...
Astrolobe removes old row versions and tombstones from storage segments.
...
- Aelio health-checks Sunjet and creates missing tables.
...
- The optional `sunjet-daemon` sweeps only `runtime_state` and can compact periodically, but
...
sequenceDiagram
    ...
    Identity->>Sunjet: customers + channel addresses
    Turn->>Sunjet: find/create session
    Turn->>Sunjet: append message + conversation event
    Turn->>Sunjet: confirmation/suspension/cache probes
    Turn->>Sunjet: history + session summary/state
    Turn->>Semantic: temporal/pathway/stance
    Semantic->>Sunjet: memories + archetypes + aspects + axes
    Turn->>Sunjet: capability mirror/binding cache
    Turn->>Harness: composed context + selected tools
    Harness->>Sunjet: ledger + suspension + traces
    Harness->>SDK: approved invocation
    SDK-->>Harness: result
    Harness->>Sunjet: ledger + function audit
    Turn->>Sunjet: intent/pathway/stance/resolution metadata
    Turn->>Sunjet: axis occurrence + assistant message
    Turn->>Sunjet: async memory/aspect learning + optional cache
```
...
Astrolobe edge columns point to physical row IDs in the same table. Cross-domain graph
...
This is unsafe if multiple tenants share one Sunjet database.
...
6. compact Astrolobe segments after logical deletion;
...
The current implementation proves that Sunjet can hold the full harness runtime:
...
Sunjet remains the storage and retrieval substrate. The harness remains the control plane.
...
packages/sunjet-client/src/types.ts
packages/sunjet-client/src/client.ts
...
Sunjet/Astrolobe/crates/ll-server
Sunjet/Astrolobe/crates/ll-query
Sunjet/Astrolobe/crates/ll-engine
```
# Harness Sunjet Data Model

This document defines the storage spine for the self-learning conversation
harness: what lives in Sunjet, which table/space owns it, how it is keyed, which
...
```text
Sunjet stores the durable truth, fast search surfaces, graph continuity, and
audit trail.
...
Sunjet tables support these column kinds today:
...
| `query.semantic` | query-by-text embedding through Sunjet where available |
...
### 1. What is Sunjet's actual isolation unit?

Sunjet's actual API isolation unit is the **table**. The HTTP client routes every
...
Inside Astrolobe today, the database facade is a **single-node database with one
...
| One Sunjet database/daemon per tenant | strongest operational split | highest | use for enterprise/high-compliance tenants |
...
### 2. Does Sunjet support filtered vector search natively?

Yes. Sunjet/Astrolobe's query executor accepts vector, text, scalar filters, and
...
Sunjet estimates predicate selectivity, picks the cheapest strategy expected to
...
Sunjet graph query is a forward reachability constraint:
...
Sunjet has durable WAL-backed row mutations and crash-safe flush/manifest
...
Required future Sunjet feature for stronger guarantees:
...
There are currently **26 configured Sunjet tables**.
...
Every table needs a predictable logical key, even when Sunjet row ids are used
...
One table stores both root rows and occurrence rows so Sunjet graph edges remain
intra-table.
...
The prompt factory should only see `ContextItem[]`, never raw Sunjet rows.
...
If Sunjet graph remains per-table only, this can be folded into
