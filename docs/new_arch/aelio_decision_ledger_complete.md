...
## 15. The tenant declaration model

What a tenant registers. Five kinds, and this is the whole surface of the product from their side.

| Kind | What it is | Notes |
|---|---|---|
| **Tools** | the callable surface from their own system | arbitrary; schema unknown in advance; see §6 and Part V |
| **Personalities** | how the agent presents itself — tone, register, framing, per tenant or per segment | constrains `Express.*` **only**; never enters retrieval scoring, policy, or σ |
| **States** | the user's position in a lifecycle | not passive labels: each carries a **permission envelope** (what is reachable from here) and a **direction** (where the system wants to move this user) |
| **Policies** | conditional gates over tool calls, state transitions, flow activation | closed predicate grammar, §12 |
| **Flows** | declarative description of the intended experience: on reaching condition X, these things get updated | rails; §5 |

Everything above lives in Aelio DB as retrievable objects, matched at turn time rather than hardcoded per tenant. That is what makes the system "living": the tenant declares the world, the retrieval spine assembles the relevant slice per turn, and the LLM only speaks at the end.

**Policy applies at three points, not one:** tool invocation, state transition, and flow activation. Policy evaluation therefore happens at three distinct places in the loop, not once per turn.

---

## 16. The two loops

These must never be conflated. Different latency, different failure tolerance, different substrate rules.

### Hot loop — per turn, deterministic, latency-bound

```
sense (env, budget, session, state)
  → gate on flow context
  → triage depth
  → resolve situation σ
  → retrieve admissible abilities by contract match
  → cost-order
  → policy check (invariant, at all three points)
  → execute
  → suspend or complete
  → append ledger
```

### Cold loop — post-turn, async, latency-free

```
score outcome from real signals
  → attribute credit per step
  → write proposal to ledger
  → promote when evidence clears the gate
  → cascade invalidation on registry / prompt change
  → schedule exploration
```

**The LLM appears only inside L1 sandwiches and at terminal synthesis.** Never in selection, never in the cold loop's promotion decision. The cold loop is pure arithmetic over recorded signals.

---

## 17. Turn ordering invariants

Established while tracing `"Hi"`. These are ordering constraints, not components, and they are easy to get wrong.

| # | Invariant | Rationale |
|---|---|---|
| O1 | **Sense before classify.** `Sense.Session` (open loops, active flow, pending step, last_seen) reads before any classification | you cannot classify an utterance without knowing whether the system is mid-sentence with this person |
| O2 | **Flow context outranks utterance semantics.** If `pending_step` is present, route to resume/advance before triage — deterministic check, no model, no embedding | "Hi" from a user parked mid-OTP is a resume signal, not a greeting. Getting this wrong is the commonest way these systems feel amnesiac. |
| O3 | Sense **reads** state; it does not mutate it | avoids the known bug class where session activity is touched before temporal resolution, corrupting the temporal scope |
| O4 | `SplitClauses` pre-gate evaluates **before** the ability is invoked | top-of-turn cost is otherwise paid unconditionally |
| O5 | Pre-gate thresholds must stay tight | `"Hi, I need to cancel my order"` must not be swallowed as a greeting; set the word-count/length threshold small |
| O6 | **A bare greeting opens no loop** | otherwise the proactive daemon nudges people who said hello and left — precisely the behavior that makes a tenant's customers hate the product |
| O7 | Situation-key σ is computed **after** triage, not before | depth is cheap and narrows the retrieval space |

---

## 18. Message lifecycle taxonomy

Three depths, and within shallow, two distinct kinds.

| Depth | Kinds | Cost | Handling |
|---|---|---|---|
| **Shallow** | *generic* — greeting, compliment, thanks, "what can you do"<br>*state-specific* — "what's my name", "how long since the last message", "what's the time" | 0 LLM | embedding match against a per-tenant surface of known shapes → `Express.Template` |
| **Boundary** | answerable from state + world knowledge, no tools | 1 LLM | `Sense.*` → `Express.Synthesize` |
| **Deep** | tools, recall, multi-step | full executor | ledger, suspension, the tier ladder |

**Depth must be decided before it is paid for.** Classify at the cheapest substrate that can decide; escalate only on low margin.

**On the LLM deciding depth when unsure** — permissible only under two constraints:
1. It proposes **depth, not path** — a closed vocabulary (`shallow | boundary | deep`, plus capability class if deep).
2. Every such call writes a proposal to the ledger keyed on σ.

The LLM is the **cold-start mechanism for routing, not the routing mechanism**. After N confirmations the semantic layer answers free forever. If it stays in the hot path permanently, the latency and cost argument is lost.

---

## 19. Versioning, migration, and the suspended-instance problem

Ability versions, prompt hashes, procedure versions, tool specs, and flow specs all change independently. A suspended flow instance may resume into a world where its referenced version no longer exists.

| # | Decision | Rationale |
|---|---|---|
| N1 | A suspended instance **pins the versions it started with** — flow version, procedure versions, prompt hashes, tool spec version | |
| N2 | On resume, pinned versions are checked for validity. Still valid → continue. Invalidated → the flow's declared **escape** fires; it does not silently run against changed semantics | a two-hour-parked OTP flow resuming into a redeployed tool is a real scenario |
| N3 | Version records are **retained while any live instance pins them**, not garbage-collected on supersession | |
| N4 | Registry change → `Registry.Invalidate` → cascade over `tool_deps` **and** `prompt_hash` dependents → affected procedures demoted → tenant may enter `rebake` | |
| N5 | Schema migration runner is required from day one and must be **per-tenant-table aware** | S1/S4 make migration N×26 |

---

## 20. Cost accounting and attribution

| # | Decision | Rationale |
|---|---|---|
| K1 | Cost recorded **per tenant, per turn, per ability**: tokens, wall-ms, tool calls, storage writes | it is the pricing model's input and the abuse-detection signal; retrofitting is miserable |
| K2 | `Budget` combinator nests; nested budgets **intersect, never widen** | a learned procedure cannot grant itself more headroom |
| K3 | Budget exhaustion is a typed outcome (`BudgetExceeded`), which triggers `DegradeGracefully`, not a crash | |
| K4 | Cost per tier is tracked separately so the tier-0/1 share can be priced | the whole efficiency argument is measurable or it is marketing |

---

## 21. Observability and human-in-the-loop surface

This is a product surface, not logging.

| # | Decision |
|---|---|
| H1 | Tenant can inspect a full **turn trace**: every step, its inputs, its outputs, why each was selected |
| H2 | Tenant reviews and approves **pending promotions**; effectful ones require it (P5) |
| H3 | Tenant can inspect any **learned procedure**: its path, its evidence, its version history, its `tool_deps` |
| H4 | **Kill switch** per procedure and per flow — immediate demotion without redeploy |
| H5 | Evidence is queryable **outside** trace-payload JSON — included *and* excluded, with scores, so retrieval decisions can be audited and calibrated |
| H6 | `Policy.Explain` returns the predicate trace for any allow/deny |
| H7 | Health surface: escalation rate per ability, tier-0/1 hit rate, tier-3 rate, disagreement rate, cost per turn |

---

## 22. Deployment topology — open

Not yet decided; needs resolution before Part XII of the architecture document.

- SDK sits in the tenant's backend and registers tools. Push, pull, or both? What is the refresh/heartbeat contract?
- Aelio server holds the registry, the harness, and Aelio DB. Single node today (Aelio DB is single-node with one WAL) — what is the path to horizontal scale, given per-tenant tables?
- Channel adapters (web widget now; WhatsApp/voice later) sit **outside** the core as ingress/egress. Confirmed direction (P3), but the adapter contract is unspecified.
- Where does the scheduler that resumes `Park`ed instances run, and how does it lease jobs before CAS lands? (§S6 single-writer is the interim answer, but it needs stating as topology.)

---

## 23. Worked traces to include as appendices

Each one was traced in discussion and should be reproduced end-to-end in the document.

| Trace | What it demonstrates |
|---|---|
| `"Hi"` — cold path | tier-3 proposal, type-check, ledger write, first-ever cost |
| `"Hi"` — warm path | zero LLM calls, ~2–4ms, template dispatch, direction-aware bindings |
| `"Hi"` — user parked mid-flow | O2: flow context outranking utterance semantics |
| `login` / OTP | activation gate, `registry.lookup` resolving rather than deciding, declared continuation, `Park`, resume with re-check, `learnable: false` |
| `"ten most avaricious clients"` | polarity-anchored term resolution, query-plan fragment, the antonym trap |
| `"Hi, what's my order status?"` | triage split across clauses, one clause escalating to deep, tool-call block firing |
...
## 25. Proposed architecture-document outline
