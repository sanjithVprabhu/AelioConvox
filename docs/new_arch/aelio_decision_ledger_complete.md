...
Everything above lives in Sunjet as retrievable objects, matched at turn time rather than hardcoded per tenant. That is what makes the system "living": the tenant declares the world, the retrieval spine assembles the relevant slice per turn, and the LLM only speaks at the end.
...
- Aelio server holds the registry, the harness, and Sunjet. Single node today (Sunjet is single-node with one WAL) — what is the path to horizontal scale, given per-tenant tables?
