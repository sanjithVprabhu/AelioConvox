...
**Aelio is a typed agent OS.** It runs agents as **executable pipelines** (flows) over a small, closed instruction set. Stages communicate through a shared JSON-shaped language (**Sol Contracts**). Durable knowledge — state, memory, learned converters, promoted procedures — lives in **Sunjet**, a multimodal store. The **LLM is a privileged author and cold-path helper**: it proposes routines and translations when the system does not yet know how; after typecheck and promotion, the same work runs **without** calling the model again.
...
**Aelio** = Agent Operating System + DSL + runtime + Sunjet-backed memory/learning.
...
| **Filesystem** | Sunjet / Astrolobe — multimodal rows (columnar, text, vector, graph) |
...
├─────────────────────────────────────────────────────────────┤
│  Sunjet       State · memory · messages · converters · procs│
├─────────────────────────────────────────────────────────────┤
...
2. Hydrate          load states, flow_instances, session, loops from Sunjet
...
This is how “the system gets life”: **meanings and glues accumulate in Sunjet**.
...
Sense is a **Sol snapshot**, not a Sunjet file format. Persistence is in tables; Sense **reads**.
...
3. If **unauthorised** — choose **conversational direction** among declared pathways:
   ...
   - Divert topic  
4. Pathway choice uses **signals** (urgency, turn gaps) + **conversation context Sol** (day / hour / 10m / 5m / last turn) + **vector search** over pathway prototypes authored once with the LLM and stored in Sunjet  
...
| Hosted agent clouds | Lock-in, opaque prompts | Open contract + self-host Sunjet |
...
| Sunjet | Real engine; tenant/CAS/retention gaps documented |
...
3. **Sol.Convert production** — Sunjet graph + cold propose  
...
- **What:** Open-source agent operating system with Sol DSL and Sunjet memory/learning.  
