import type {
  ...
  ScanResponse,
  SchemaResponse,
  AelioDbClientOptions,
} from './types.js';

export class AelioDbHttpError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: string,
    message?: string,
  ) {
    super(message ?? `Aelio DB HTTP ${status}: ${body}`);
    this.name = 'AelioDbHttpError';
  }
}

export class AelioDbClient {
  private readonly baseUrl: string;
  private readonly apiKey?: string;
  private readonly timeoutMs: number;

  constructor(options: AelioDbClientOptions) {
  ...
  private async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new AelioDbHttpError(response.status, text);
      }
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new Error(`AelioDb request timed out after ${this.timeoutMs}ms`);
      }
      throw error;
    } finally {
      clearTimeout(timer);
