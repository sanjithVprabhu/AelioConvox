...
//! strategy (meeting the recall bar) on the real `aelio-db-index` engine.

use aelio_db_cost::{choose, Histogram, Strategy, VecCostParams};
use aelio_db_index::{
...
fn cost_model_picks_the_empirically_optimal_strategy() {
    ...
    let params = VecCostParams {
        n: N,
        k: K,
        ef: EF,
        hnsw_base_evals: base,
        base_ann_recall: 1.0,
    };
