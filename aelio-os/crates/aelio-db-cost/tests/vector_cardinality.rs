...
use aelio_db_cost::VectorStats;
use aelio_db_index::SplitMix64;
...
fn estimates_are_accurate_and_discriminative() {
    ...
    assert!(
        est_lo < est_mid && est_mid < est_hi,
        "estimates not monotone: {est_lo:.3} {est_mid:.3} {est_hi:.3}"
    );
    assert!(
        est_lo < 0.2,
        "selective predicate over-estimated: {est_lo:.3}"
    );
    assert!(
        est_hi > 0.4,
        "non-selective predicate under-estimated: {est_hi:.3}"
    );
}
