...
pub enum SegmentBackend {
    /// Segments under the configured Aelio data directory (default).
    Local,
...
impl StorageConfig {
    /// Load from the canonical `AELIO_DB_*` deployment environment.
    pub fn from_env(data_dir: PathBuf) -> io::Result<Self> {
        let backend_raw =
            first_env(&["AELIO_DB_SEGMENT_BACKEND"]).unwrap_or_else(|| "local".into());

        let backend = SegmentBackend::parse(&backend_raw).ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("unknown segment backend '{backend_raw}' — use local|s3"),
            )
        })?;

        let prefix = first_env(&["AELIO_DB_SEGMENT_PREFIX"]).unwrap_or_default();
        let prefix = normalize_prefix(&prefix);

        let s3_bucket = first_env(&["AELIO_DB_S3_BUCKET"]);
        let s3_region = first_env(&["AELIO_DB_S3_REGION"]);
        let s3_endpoint = first_env(&["AELIO_DB_S3_ENDPOINT"]);
        let s3_access_key_id = first_env(&["AELIO_DB_S3_ACCESS_KEY_ID", "AWS_ACCESS_KEY_ID"]);
        let s3_secret_access_key =
            first_env(&["AELIO_DB_S3_SECRET_ACCESS_KEY", "AWS_SECRET_ACCESS_KEY"]);

        let s3_allow_http = flag_env(&["AELIO_DB_S3_ALLOW_HTTP"]);
        let s3_path_style = flag_env(&["AELIO_DB_S3_PATH_STYLE"]);

        if backend == SegmentBackend::S3 && s3_bucket.as_ref().map(|s| s.is_empty()).unwrap_or(true)
        {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "S3 segment backend requires AELIO_DB_S3_BUCKET",
            ));
        }
...
pub fn from_env(
    data_dir: PathBuf,
) -> io::Result<(StorageConfig, std::sync::Arc<dyn crate::SegmentStore>)> {
