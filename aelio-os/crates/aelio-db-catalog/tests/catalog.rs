...
use aelio_db_catalog::{Catalog, CatalogError, ColumnKind, FileRef};
...
impl Tmp {
    fn new() -> Self {
        ...
        Tmp(std::env::temp_dir().join(format!("aelio_cat_{}_{n}.cat", std::process::id())))
    }
}
impl Drop for Tmp {
    fn drop(&mut self) {
...
fn create_and_inspect_table() {
    ...
    assert_eq!(
        t.column("embedding").unwrap().kind,
        ColumnKind::Vector(1536)
    );
...
fn persistence_roundtrip() {
    ...
    cat.create_table(
        "users",
        &[("name", ColumnKind::Utf8), ("vec", ColumnKind::Vector(8))],
    )
    .unwrap();
    ...
    let id = loaded
        .create_table("third", &[("a", ColumnKind::Bool)])
        .unwrap();
    assert_eq!(id, 3);
}
