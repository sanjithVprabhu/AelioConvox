...
use aelio_db_api::nl::{BoxFuture, LlmClient};
use aelio_db_api::{router, AppState};
...
fn fresh_db(tag: &str) -> Database {
    let dir = std::env::temp_dir().join(format!("aelio_api_{tag}_{}", std::process::id()));
...
async fn table_row_lifecycle_and_hybrid_query() {
    ...
    let (_s, b) = call(
        &app,
        "DELETE",
        &format!("/v1/tables/docs/rows/{id1}"),
        k,
        None,
    )
    .await;
...
async fn semantic_clause_embeds_then_searches() {
    ...
    let state = AppState::new(fresh_db("sem"), vec![])
        .with_embedder(Arc::new(MockEmbedder(vec![1.0, 0.0])));
    let app = router(state);
    let k = None;
    ...
    call(
        ...
        "/v1/tables",
        k,
        Some(json!({"name":"docs","columns":[{"name":"emb","kind":"vector","dim":2}]})),
    )
    .await;
    let (_s, near) = call(
        &app,
        ...
        "/v1/tables/docs/rows",
        k,
        Some(json!({"values":{"emb":{"type":"vector","value":[1.0,0.0]}}})),
    )
    .await;
    ...
    call(
        ...
        "/v1/tables/docs/rows",
        k,
        Some(json!({"values":{"emb":{"type":"vector","value":[10.0,0.0]}}})),
    )
    .await;
    ...
    let results = body["results"].as_array().unwrap();
    assert_eq!(
        results[0]["row_id"].as_u64().unwrap(),
        id_near,
        "semantic search ranked wrong row: {body}"
    );
}

#[tokio::test]
async fn embed_on_insert_stores_a_vector() {
    let state = AppState::new(fresh_db("emb_ins"), vec![])
        .with_embedder(Arc::new(MockEmbedder(vec![1.0, 0.0])));
    let app = router(state);
    let k = None;
    ...
    let (_s, body) = call(
        ...
        "/v1/tables/docs/query",
        k,
        Some(json!({"k":1,"vector":{"col":"emb","query":[1.0,0.0]}})),
    )
    .await;
    assert_eq!(
        body["results"][0]["row_id"].as_u64().unwrap(),
        id,
        "embedded row not found: {body}"
    );
}
...
async fn semantic_dimension_mismatch_is_400() {
    ...
    // empty result.
    let state = AppState::new(fresh_db("semdim"), vec![])
        .with_embedder(Arc::new(MockEmbedder(vec![1.0, 0.0])));
    let app = router(state);
    call(
        &app,
        ...
        "/v1/tables",
        None,
        Some(json!({"name":"d","columns":[{"name":"e","kind":"vector","dim":3}]})),
    )
    .await;
    let (status, body) = call(
        &app,
        ...
        "/v1/tables/d/query",
        None,
        Some(json!({"k":1,"semantic":{"col":"e","text":"x"}})),
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST, "{body}");
    assert!(
        body["error"].as_str().unwrap().contains("3-dimensional"),
...
async fn embed_on_insert_dimension_mismatch_is_400() {
    let state = AppState::new(fresh_db("insdim"), vec![])
        .with_embedder(Arc::new(MockEmbedder(vec![1.0, 0.0])));
    let app = router(state);
    call(
        &app,
    ...
    let (status, _) = call(
        ...
        "/v1/tables/d/rows",
        None,
        Some(json!({"values":{"e":{"type":"embed","value":"x"}}})),
    )
    .await;
...
async fn semantic_without_embedder_is_503() {
    ...
    call(
        ...
        "/v1/tables",
        None,
        Some(json!({"name":"d","columns":[{"name":"e","kind":"vector","dim":2}]})),
    )
    .await;
    let (status, _) = call(
        &app,
...
#[tokio::test]
async fn prism_endpoint_parses_embeds_executes_and_projects() {
    let state = AppState::new(fresh_db("prism"), vec![])
        .with_embedder(Arc::new(MockEmbedder(vec![1.0, 0.0])));
    let app = router(state);
    call(
        &app,
    ...
    call(
        ...
        None,
        Some(json!({"values":{
            "id":{"type":"utf8","value":"d1"},
            "body":{"type":"utf8","value":"customer reply guidance"},
            "embedding":{"type":"vector","value":[1.0,0.0]},
            "tenant_secret":{"type":"utf8","value":"must-not-leak"}
        }})),
    )
    .await;
    ...
    assert_eq!(body["results"][0]["fields"]["id"]["value"], "d1");
    assert!(body["results"][0]["fields"].get("tenant_secret").is_none());
    ...
    let (status, body) = call(
        ...
        Some(json!({
            "from":"docs",
            "match":{"kind":"text","on":"body","query":"x","boost":99},
            "select":["id"],
            "limit":1
        })),
    )
...
async fn nl_is_503_when_no_backend_configured() {
    ...
    call(
        ...
        "/v1/tables",
        None,
        Some(json!({"name":"t","columns":[{"name":"b","kind":"text"}]})),
    )
    .await;
    let (status, _) = call(
        &app,
