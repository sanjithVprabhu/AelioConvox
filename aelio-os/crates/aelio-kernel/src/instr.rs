...
pub fn parse_node(j: &J) -> Result<Node, ErrV1> {
    let obj = j
        .as_object()
        .ok_or_else(|| shape("?", "instruction must be a JSON object"))?;
    let nid = obj
        .get("nid")
        .and_then(J::as_str)
    ...
    let kind = match op {
        ...
        "Let" => {
            let bindings = obj
                ...
                .map(|b| {
                    let binding = b
                        .as_object()
                        .ok_or_else(|| shape(&nid, "Let binding must be an object"))?;
                    exact_fields(&nid, "Let binding", binding, &["key", "value"])?;
                    let key = b
                        .get("key")
                        .and_then(J::as_str)
                        .ok_or_else(|| shape(&nid, "binding.key"))?;
        ...
        "Branch" => Kind::Branch {
            ...
            els: obj
                .get("else")
                .map(|e| parse_node(e).map(Box::new))
                .transpose()?,
        },
        "Loop" => Kind::Loop {
        ...
        "Try" => {
            ...
            Kind::Try {
                ...
                err_into: path(&nid, obj.get("err_into"), "err_into")?,
                    ...
                    .get("finally")
                    .map(|f| parse_node(f).map(Box::new))
                    .transpose()?,
            }
        }
        "Fallback" => {
        ...
        "Guard" => Kind::Guard {
            ...
            on_violation: obj
                .get("on_violation")
                .map(|v| parse_node(v).map(Box::new))
                .transpose()?,
        ...
        "Once" => Kind::Once {
            ...
            idem_key: match obj.get("idem_key") {
                ...
                Some(k) => {
                    let key_obj = k
                        .as_object()
                        .ok_or_else(|| shape(&nid, "Once.idem_key must be an object"))?;
                    exact_fields(&nid, "Once.idem_key", key_obj, &["template"])?;
                    let arr = k.get("template").and_then(J::as_array).ok_or_else(|| {
                        shape(&nid, "Once.idem_key must be {template:[Expr,...]}")
                    })?;
                    Some(
                        arr.iter()
                            .map(|e| parse_expr(&nid, Some(e)))
                            .collect::<Result<_, _>>()?,
                    )
                }
            },
        ...
        "Park" => {
            let until = obj
                .get("until")
                .and_then(J::as_object)
                .ok_or_else(|| shape(&nid, "Park.until"))?;
            let until_kind = until
                .get("kind")
                .and_then(J::as_str)
                .ok_or_else(|| shape(&nid, "Park.until.kind"))?;
            let allowed = match until_kind {
                "event" => &["kind"][..],
                "ttl" => &["kind", "ms"][..],
                "instant" => &["kind", "at"][..],
                _ => return Err(shape(&nid, "Park.until.kind ∈ {event,ttl,instant}")),
            };
            exact_fields(&nid, "Park.until", until, allowed)?;
            let until = match until_kind {
                "event" => Until::Event,
                "ttl" => Until::Ttl {
                    ms: pos_int(&nid, until.get("ms"), "until.ms")?,
                },
                "instant" => Until::Instant {
                    at: until
                        .get("at")
                        .and_then(J::as_str)
                        .ok_or_else(|| shape(&nid, "instant.at"))?
                        .into(),
                },
                _ => unreachable!(),
            };
...
fn check_known_fields(nid: &str, op: &str, obj: &serde_json::Map<String, J>) -> Result<(), ErrV1> {
    ...
    for key in obj.keys() {
        ...
        if !allowed.contains(&key.as_str()) {
            return Err(shape(
                nid,
                format!("unknown field `{key}` on op `{op}` (closed schema, App E)"),
            ));
        }
    }
    Ok(())
    ...
    match j {
        ...
        Some(obj) => obj
            ...
            .map(|(k, v)| {
                let p = Path::parse(
                    v.as_str()
                        .ok_or_else(|| shape(nid, "import path must be string"))?,
                )
...
pub fn parse_expr(nid: &str, j: Option<&J>) -> Result<Expr, ErrV1> {
    ...
    if let Some(lit) = obj.get("lit") {
        exact_fields(nid, "literal expression", obj, &["lit"])?;
        return Ok(Expr::Lit(
            json::from_json(lit).map_err(|e| shape(nid, format!("lit: {e}")))?,
        ));
    }
    if let Some(p) = obj.get("pull") {
        exact_fields(nid, "pull expression", obj, &["pull"])?;
        let p = p
            .as_str()
            .ok_or_else(|| shape(nid, "pull must be a string path"))?;
        return Ok(Expr::Pull(
            Path::parse(p).map_err(|e| shape(nid, format!("pull path: {e}")))?,
        ));
    }
    if let Some(op) = obj.get("fn").and_then(J::as_str) {
        exact_fields(nid, "fn expression", obj, &["fn", "args"])?;
        let args = obj
            .get("args")
            .and_then(J::as_array)
...
fn exact_fields(
    ...
    for key in obj.keys() {
        if !allowed.contains(&key.as_str()) {
            return Err(shape(
                nid,
                format!("unknown field `{key}` in {context} (closed schema, App E)"),
            ));
        }
    }
    Ok(())
...
fn boxed(nid: &str, j: Option<&J>) -> Result<Box<Node>, ErrV1> {
    Ok(Box::new(parse_node(j.ok_or_else(|| {
        shape(nid, "missing child instruction")
    })?)?))
}
...
fn pos_int(nid: &str, j: Option<&J>, field: &str) -> Result<u64, ErrV1> {
    let n = j
        .and_then(J::as_u64)
        .ok_or_else(|| shape(nid, format!("`{field}` must be a positive int")))?;
    if n == 0 {
