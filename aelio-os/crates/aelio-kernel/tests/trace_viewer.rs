...
fn trace_shows_op_by_op_bag_writes_from_the_ledger() {
    ...
    reg.register("io.read@1", EffectClass::Read, |_| {
        Ok(SolValue::map([("loggedin", SolValue::Bool(true))]))
    });
    reg.register("tool.mark@1", EffectClass::Write, |_| {
        Ok(SolValue::map([("done", SolValue::Bool(true))]))
    });
    let mut inst = Instance::new(program.clone(), &mut reg);
    assert!(matches!(
        inst.start(SolValue::map::<_, &str>([])).unwrap(),
    ...
    assert!(
        entries
            .iter()
            .any(|e| e.nid.as_deref() == Some("n_read") && e.summary.starts_with("auth :=")),
        "trace shows auth := <io.read output>"
    );
    assert!(
        entries
            .iter()
            .any(|e| e.nid.as_deref() == Some("n_mark") && e.summary.starts_with("result :=")),
    ...
    assert!(entries
        .first()
        .map(|e| e.kind == "turn_start")
        .unwrap_or(false));
    assert!(entries
        .iter()
        .any(|e| e.kind == "turn_end" && e.summary.contains("bag_hash=")));
