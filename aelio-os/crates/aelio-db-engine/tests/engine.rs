...
use aelio_db_format::{read_file, ColumnValues};
...
impl Tmp {
    fn new(ext: &str) -> Self {
        ...
        Tmp(std::env::temp_dir().join(format!("aelio_eng_{}_{n}.{ext}", std::process::id())))
    }
}
impl Drop for Tmp {
    fn drop(&mut self) {
...
fn writes_survive_recovery() {
    ...
    // Session 2: recover from the WAL — committed writes are back, delete applied.
    {
        ...
        assert_eq!(e.live_count(), 2);
        assert_eq!(
            e.get(10).unwrap().get(&1),
            Some(&Value::Utf8("alice".into()))
        );
        assert!(
            e.get(20).is_none(),
            "deleted row stays deleted after recovery"
        );
    ...
    // Session 3: the post-recovery write is durable too.
    {
        ...
        assert_eq!(
            e.get(40).unwrap().get(&1),
            Some(&Value::Utf8("dave".into()))
        );
    }
}
...
fn flush_writes_a_vss_file() {
    ...
    let xmax = file
        .columns
        .iter()
        .find(|c| c.column_id == aelio_db_engine::SYS_XMAX_COL)
        .unwrap();
    match &xmax.values {
        ColumnValues::I64(v) => {
