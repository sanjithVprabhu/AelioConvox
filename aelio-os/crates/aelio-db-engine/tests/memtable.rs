...
fn versions_are_visible_by_snapshot_lsn() {
    ...
    assert_eq!(
        m.get(42, m.snapshot_lsn()).unwrap().get(&1),
        Some(&Value::I64(20))
    );
...
#[test]
fn sampling_is_memory_bounded_and_reports_the_visible_count() {
    let mut m = Memtable::new();
    for id in 0..10_000 {
        m.apply(id, Op::Put(row(id as i64)), id + 1);
    }
    m.apply(7, Op::Delete, 10_001);

    let (sample, total) = m.sample_row_ids(m.snapshot_lsn(), 64);
    assert_eq!(sample.len(), 64);
    assert_eq!(total, 9_999);
    assert!(!sample.contains(&7));

    let (empty_sample, same_total) = m.sample_row_ids(m.snapshot_lsn(), 0);
