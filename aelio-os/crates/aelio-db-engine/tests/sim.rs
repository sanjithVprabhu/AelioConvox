...
use aelio_db_engine::{Engine, Row, Value, SYS_XMAX_COL};
...
impl TmpDir {
    fn new() -> Self {
        ...
        let p = std::env::temp_dir().join(format!("aelio_sim_{}_{n}", std::process::id()));
...
fn run_seed(seed: u64) {
    ...
    for (id, row) in &model {
        assert_eq!(
            eng.get(*id),
            Some(row),
            "seed {seed}: row {id} after recovery"
        );
    }
    assert_eq!(
        eng.memtable().live_count(),
        model.len(),
        "seed {seed}: live count"
    );
    assert!(
        eng.get(next_id + 12345).is_none(),
        "seed {seed}: phantom row"
    );
