...
use aelio_db_graph::{serialize_edge_index, EdgeIndex};
use aelio_db_index::{encode_recall_curve, serialize_hnsw, Hnsw, HnswParams, RECALL_EF_LADDER};
use aelio_db_text::{serialize_text_index, InvertedIndex};
use aelio_db_wal::{RecordType, Wal};
...
impl Engine {
    ...
    pub fn flush_memtable_to<P: AsRef<Path>>(mem: &Memtable, vss_path: P) -> io::Result<usize> {
        ...
        let entries: Vec<(u64, Option<&Row>)> =
            visible.iter().map(|(id, _, r)| (*id, *r)).collect();
        let n = entries.len();
        ...
        let meta = FileMeta {
            ...
            writer_version: "aelio-db-engine".into(),
        };
        write_file_with(
            vss_path,
            &meta,
    ...
    for cid in cids {
        match first_value(entries, cid) {
            Some(Value::Vector(first)) => {
                ...
                let curve =
                    idx.measure_recall_curve(RECALL_PROBE_K, RECALL_EF_LADDER, RECALL_SAMPLE);
                extra.push(section(
                    SectionType::Hnsw,
                    cid,
                    serialize_hnsw(&idx, cid, &local_offsets),
                ));
                extra.push(section(
                    SectionType::OptimizerStats,
                    cid,
                    encode_recall_curve(&curve),
                ));
            }
            Some(Value::Utf8(_)) => {
...
fn first_value<'a>(entries: &'a [(u64, Option<&Row>)], cid: u32) -> Option<&'a Value> {
    entries
        .iter()
        .find_map(|(_, r)| match r.and_then(|row| row.get(&cid)) {
...
/// Convert the flush entries into aelio-db-format columns (entry order == local offset). A
