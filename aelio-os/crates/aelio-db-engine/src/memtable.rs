...
impl Memtable {
    ...
    pub fn get(&self, row_id: u64, snapshot: u64) -> Option<&Row> {
        self.rows
            .get(&row_id)
            .and_then(|vs| Self::visible_at(vs, snapshot))
    }
    ...
    /// Return at most `max` evenly distributed visible row ids plus the exact visible count.
    ///
    /// Unlike [`Self::scan`], this never allocates in proportion to the table size. Query
    /// planning uses it to estimate predicate selectivity without first materialising every
    /// row id. The pass is linear because historical snapshots do not have a precomputed live
    /// count, but its temporary memory is strictly bounded by `max`.
    pub fn sample_row_ids(&self, snapshot: u64, max: usize) -> (Vec<u64>, usize) {
        if max == 0 {
            let total = self
                .rows
                .values()
                .filter(|versions| Self::visible_at(versions, snapshot).is_some())
                .count();
            return (Vec::new(), total);
        }

        let mut sample = Vec::with_capacity(max.min(self.rows.len()));
        let mut total = 0usize;
        for (id, versions) in &self.rows {
            if Self::visible_at(versions, snapshot).is_none() {
                continue;
            }
            total += 1;
            if sample.len() < max {
                ...
                // Deterministic reservoir sampling. This keeps the sample spread over the
                // complete ordered map without requiring randomness or a second pass.
