...
use aelio_agent::provider::{OpenAiCompatibleProvider, ProviderCall};
use aelio_agent::runtime::{DurableRuntime, DurableTurnRequest};
use aelio_agent::storage::{AelioStore, LogicalTable, StoredRecord};
use aelio_agent::types::LookupTier;
...
fn print_turn(label: &str, result: &aelio_agent::blocks::turn::TurnResult) {
    eprintln!("\n=== {label} ===");
...
fn live_multi_turn_login_flow_stores_state_and_steps() {
    ...
    let flow: Option<StoredRecord<Option<aelio_agent::blocks::flow::FlowInstance>>> = rt
        .store
...
fn live_propose_path_returns_declared_abilities_only() {
    ...
    use aelio_agent::abilities::learn::{propose_path_with_provider, situation_key};
    use aelio_agent::provider::LlmProvider;
    ...
    let (path, prompt_hash) =
        propose_path_with_provider(&sigma, "", &abilities, &abilities, &mut provider)
            .expect("live propose_path");
...
fn live_bigger_composition_scenario_end_to_end() {
    ...
    let all_steps: Vec<StoredRecord<aelio_agent::runtime::DurableStepAttempt>> = rt
        .store
        .list("live-tenant", LogicalTable::StepAttempts, None, 512)
        .unwrap();
    let all_calls: Vec<StoredRecord<aelio_agent::runtime::TurnProviderCallRecord>> = rt
        .store
