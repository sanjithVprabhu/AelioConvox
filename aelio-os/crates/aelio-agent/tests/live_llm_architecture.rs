...
use aelio_agent::storage::{AelioStore, LogicalTable, StoredRecord};
use aelio_agent::tenant::{OutputSpec, ToolSpec};
use aelio_agent::types::TenantMode;
use aelio_agent::{LookupTier, ReasonCode, Value};
...
fn validates_live_llm_architecture_without_hiding_gaps() {
    ...
    let (proposed, proposal_hash) =
        propose_path_with_provider(&sigma, "", &declared, &declared, &mut proposal_provider)
    ...
    let (composed, _) = propose_path_with_provider(
        &composed_sigma,
        "",
        &arbitrary_ids,
        &arbitrary_ids,
        &mut composed_provider,
    )
    ...
    let mut effects = aelio_agent::ops::effects::EffectEnv::live();
    ...
    let malformed = LlmResponse {
        content: r#"{"steps":[{"ability_id":"Unsupported.Exfiltrate","args":{}}]}"#.into(),
    ...
    let malformed_result =
        propose_path_with_provider(&sigma, "", &declared, &declared, &mut malformed_provider);
    ...
    // 6-8: run the actual World/DurableRuntime, inspect Aelio DB through Rust APIs, then reopen.
    ...
    let active_before_resume: StoredRecord<Option<aelio_agent::blocks::flow::FlowInstance>> =
        reopened
            .store
    ...
    for (name, table) in [
        ...
        checks.push(Check {
            ...
            detail: format!("Aelio DB row count after durable cold turn: {count}"),
        });
    }
    ...
    // 9: two workers sharing one Aelio DB store race the same turn ID. Exactly one effect wins.
