...
/// Recognize an explicit user instruction or direct first-person preference assertion to persist
/// a fact. Arbitrary conversation is never silently promoted into durable personal memory.
pub fn explicit_fact(utterance: &str) -> Option<&str> {
    ...
    // "I prefer …" is itself a clear, user-authored preference declaration. Preserve the full
    // assertion so recall has enough subject and predicate context; do not generalize this to
    // broad "I am"/"my …" extraction, which could silently retain sensitive incidental speech.
    if lower.starts_with("i prefer ") && !trimmed["i prefer ".len()..].trim().is_empty() {
        return Some(trimmed);
    }
    None
}
...
#[cfg(test)]
mod explicit_memory_tests {
    use super::*;

    #[test]
    fn recognizes_instruction_and_direct_preference_but_not_incidental_speech() {
        assert_eq!(
            explicit_fact("remember that my locale is en-IN"),
            Some("my locale is en-IN")
        );
        assert_eq!(
            explicit_fact("I prefer metric units"),
            Some("I prefer metric units")
        );
        assert_eq!(explicit_fact("I discussed metric units yesterday"), None);
        assert_eq!(explicit_fact("I prefer"), None);
    }
}
