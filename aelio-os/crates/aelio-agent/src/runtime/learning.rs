...
impl LearningColdLoop {
    ...
    pub fn runner_up(
        ...
        // The database enforces a universal 10,000-row query cap. Treat a full page as
        // potentially truncated and fail closed rather than bypassing that cap with a sentinel
        // request of 10,001.
        if procedures.len() == 10_000 {
            return Err(AelioError::new(
                ReasonCode::BudgetExceeded,
                "procedure catalog reached the bounded runner-up scan limit",
            ));
        }
