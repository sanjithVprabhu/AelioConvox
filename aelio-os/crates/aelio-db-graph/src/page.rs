...
impl<'a> EdgeView<'a> {
    ...
    pub fn out_neighbors(&self, s: u32) -> Vec<u64> {
        ...
        let a = rd_u32(
            self.bytes,
            self.fwd_offsets_off.saturating_add(s as usize * 4),
        ) as usize;
        let b = rd_u32(
            self.bytes,
            self.fwd_offsets_off.saturating_add((s as usize + 1) * 4),
        ) as usize;
        ...
        while lo < hi {
            ...
            match v.cmp(&t) {
                std::cmp::Ordering::Equal => {
                    let a =
                        rd_u32(self.bytes, self.rev_offsets_off.saturating_add(mid * 4)) as usize;
                    let b = rd_u32(
                        self.bytes,
                        self.rev_offsets_off.saturating_add((mid + 1) * 4),
                    ) as usize;
