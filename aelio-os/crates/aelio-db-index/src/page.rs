...
pub fn serialize_hnsw(index: &Hnsw, column_id: u32, local_offsets: &[u32]) -> Vec<u8> {
    ...
    assert_eq!(
        local_offsets.len(),
        n,
        "local_offsets must cover every node"
    );
    ...
    let entry_point = index.entry().map(|e| old_to_new[e as usize]).unwrap_or(0);
    ...
    let mut out =
        Vec::with_capacity(HEADER_LEN + quant_len as usize + slots.len() + upper_blob.len());
...
impl<'a> HnswView<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self, IndexError> {
        ...
        let quant_bytes = dim
            .checked_mul(8)
            .ok_or(IndexError::Truncated("quant size"))?;
        ...
        let scale: Vec<f32> = (0..dim)
            .map(|d| rd_f32(bytes, qscale_off + d * 4))
            .collect();
    ...
    fn neighbors(&self, id: u32, layer: usize, out: &mut Vec<u32>) {
        ...
        let Some(blob) = self.bytes.get(self.upper_off..) else {
            return;
        };
        ...
        while lo < hi {
            let mid = (lo + hi) / 2;
            let Some(entry_id) = g32(blob, index_base + mid * 8) else {
                return;
            };
        ...
        for l in 1..=level {
            ...
            if l == layer {
                for i in 0..cnt {
                    let Some(v) = g32(blob, p + i * 4) else {
                        return;
                    };
                    out.push(v);
                }
