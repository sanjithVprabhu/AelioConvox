...
fn postfilter_collapses_but_integrated_is_robust_at_low_selectivity() {
    ...
    assert!(
        post_r < 0.5,
        "expected post-filter to collapse, got {post_r:.3}"
    );
    assert!(
        int_r >= 0.95,
        "expected integrated to stay robust, got {int_r:.3}"
    );
}
...
fn cost_optimal_strategy_depends_on_selectivity() {
    ...
    assert!(
        (pre_hi - N as f64).abs() < 1.0,
        "prefilter at s=1.0 should scan all N"
    );
    assert!(
        post_hi < pre_hi,
    ...
    assert!(
        post_r_hi >= 0.9,
        "post-filter recall at s=1.0 was {post_r_hi:.3}"
    );
}
