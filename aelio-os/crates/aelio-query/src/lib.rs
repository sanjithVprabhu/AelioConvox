...
impl DatasetRegistry {
    pub fn declare(&mut self, tenant: impl Into<String>, dataset: Dataset) {
        self.datasets.insert((tenant.into(), dataset.id.clone()), dataset);
    }

    pub fn get(&self, tenant: &str, dataset: &str) -> Option<&Dataset> {
        self.datasets.get(&(tenant.to_string(), dataset.to_string()))
    }
}
...
pub fn parse_query(j: &J) -> Result<QueryAst, String> {
    ...
    let dataset = o.get("dataset").and_then(J::as_str).ok_or("query missing `dataset`")?.to_string();
    let qop = QOp::parse(o.get("qop").and_then(J::as_str).ok_or("query missing `qop`")?)
        .ok_or("unknown `qop` — closed set (§10.4)")?;
    let params = o.get("params").map(sol).transpose()?.unwrap_or_else(|| SolValue::map::<_, &str>([]));
    let limit = o
        .get("limit")
        .and_then(J::as_u64)
        .filter(|&n| n > 0)
        .ok_or("query `limit` is mandatory and must be > 0 (§10.4 → G1)")?;

    let (max_depth, max_nodes) = if qop == QOp::Traverse {
        let d = o.get("max_depth").and_then(J::as_u64).filter(|&n| n > 0)
            .ok_or("traverse requires positive `max_depth` (§10.4)")?;
        let n = o.get("max_nodes").and_then(J::as_u64).filter(|&n| n > 0)
            .ok_or("traverse requires positive `max_nodes` (§10.4)")?;
        (Some(d), Some(n))
    } else {
        (None, None)
    };

    Ok(QueryAst { dataset, qop, params, limit, max_depth, max_nodes })
}
...
pub fn check_admissible(reg: &DatasetRegistry, tenant: &str, q: &QueryAst) -> Result<(), String> {
    let ds = reg
        .get(tenant, &q.dataset)
        .ok_or_else(|| format!("dataset `{}` not declared for tenant `{tenant}` (§17.2)", q.dataset))?;
    if !ds.modalities.contains(&q.qop) {
        return Err(format!("dataset `{}` does not support {:?} (§10.4)", q.dataset, q.qop));
    }
    Ok(())
}
