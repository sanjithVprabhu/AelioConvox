...
use aelio_db_engine::{Memtable, Value};
use aelio_db_text::tokenize;

use crate::util::{l2_sq, top_k};

/// BM25 constants (match `aelio-db-text`).
...
pub trait Source: Sync {
    ...
    /// At most `max` representative visible row ids and the exact visible-row count.
    /// Implementations must keep temporary memory bounded by `max`; the planner calls this
    /// instead of `all_rows` when estimating selectivity.
...
impl Source for Memtable {
    ...
    fn sample_rows(&self, snapshot: u64, max: usize) -> (Vec<u64>, usize) {
        self.sample_row_ids(snapshot, max)
    }
