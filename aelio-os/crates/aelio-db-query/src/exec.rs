...
use aelio_db_cost::{choose, Strategy, VecCostParams};
...
pub fn execute_checked(
    ...
    let fetch = q.k.max(1).saturating_mul(4);
...
/// bounded sample and let `aelio-db-cost` weigh post-filter (at the smallest `ef` meeting the recall
...
fn estimate_selectivity(
    ...
    for s in sources {
        let (rows, source_total) = s.sample_rows(snapshot, SAMPLE);
...
fn post_fetch(k: usize, sel: f64, total: usize) -> usize {
    let floor = k.saturating_mul(8).min(total.max(1));
    if sel <= 0.0 {
        return floor;
    }
