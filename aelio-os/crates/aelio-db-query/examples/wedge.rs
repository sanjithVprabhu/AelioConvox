...
//!
//! Run: `cargo run --release -p aelio-db-query --example wedge`
//!
...
fn main() {
    let dir: PathBuf = std::env::temp_dir().join(format!("aelio_wedge_{}", std::process::id()));
    ...
    let id4 = db
        .insert(
            ...
            &paper(
                [1.0, 0.0, 0.0],
                "denoising diffusion probabilistic methods",
                vec![_id2],
                2022,
            ),
        )
    ...
    let _id6 = db
        .insert(
            ...
            &paper(
                [0.0, 0.0, 1.0],
                "protein folding with graph networks",
                vec![],
                2022,
            ),
        )
    ...
    let _id7 = db
        .insert(
            ...
            &paper(
                [0.0, 0.0, 1.0],
                "heat diffusion in commercial cooking",
                vec![],
                2023,
            ),
        )
    ...
    let id8 = db
        .insert(
            ...
            &paper(
                [1.0, 0.0, 0.0],
                "practical tricks for diffusion sampling",
                vec![],
                2024,
            ),
        )
    ...
    let preprint = db
        .insert(
            ...
            &paper(
                [0.98, 0.0, 0.0],
                "fast samplers for latent diffusion",
                vec![id5, id8],
                2024,
            ),
        )
    ...
    for (id, score) in &results {
        let origin = if *id == id8 {
            "MEMTABLE (read-your-writes)"
        } else {
            "segment (persisted)"
        };
    ...
    println!(
        "    (preprint -> paper {id5} in the segment, and -> paper {id8} still in the memtable)"
    );
    ...
    println!(
        "  - read-your-writes: paper {id8} was inserted moments ago, never flushed, yet it is"
    );
    println!(
        "    a first-class participant — no reindex, no separate write path, one consistent plan."
    );
