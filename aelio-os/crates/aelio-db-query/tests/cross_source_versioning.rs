...
use aelio_db_query::{execute, FileSource, Query, Source};
...
fn deleted_after_flush_does_not_resurface_from_segment() {
    let dir = std::env::temp_dir().join(format!("aelio_xsrc_{}", std::process::id()));
    ...
    let res = execute(
        &sources,
        &Query::new(2).with_vector(VEC, vec![0.0, 0.0]),
        SNAP,
    );
...
fn updated_after_flush_uses_the_new_version_not_the_stale_segment_copy() {
    let dir = std::env::temp_dir().join(format!("aelio_xupd_{}", std::process::id()));
    let _ = std::fs::create_dir_all(&dir);
    let wal = dir.join("wal.log");
    let seg = dir.join("seg.vss");
    ...
    let res = execute(
        &sources,
        &Query::new(1).with_vector(VEC, vec![0.0, 0.0]),
        SNAP,
    );
    let _ = std::fs::remove_dir_all(&dir);
...
fn delete_survives_a_second_flush() {
    ...
    let dir = std::env::temp_dir().join(format!("aelio_x2f_{}", std::process::id()));
    let _ = std::fs::create_dir_all(&dir);
    let wal = dir.join("wal.log");
