...
fn vector_search_sees_recent_writes_and_mvcc() {
    ...
    put(&mut m, 1, 1, |r| {
        r.insert(VEC, Value::Vector(vec![0.0, 0.0]));
    });
    put(&mut m, 2, 2, |r| {
        r.insert(VEC, Value::Vector(vec![10.0, 0.0]));
    });
    put(&mut m, 3, 3, |r| {
        r.insert(VEC, Value::Vector(vec![5.0, 0.0]));
    });

    // Nearest to (4.5, 0) is row 3 (at 5,0), then row 1 (0,0).
    let res = m.vector_search(VEC, &[4.5, 0.0], 2, 64, m.snapshot_lsn());
    assert_eq!(
        res.iter().map(|(id, _)| *id).collect::<Vec<_>>(),
        vec![3, 1]
    );
    ...
    put(&mut m, 3, 4, |r| {
        r.insert(VEC, Value::Vector(vec![99.0, 0.0]));
    });
...
fn text_search_sees_recent_writes_and_deletes() {
    let mut m = Memtable::new();
    put(&mut m, 1, 1, |r| {
        r.insert(BODY, Value::Utf8("quick brown fox".into()));
    });
    put(&mut m, 2, 2, |r| {
        r.insert(BODY, Value::Utf8("quick quick volatility".into()));
    });
    put(&mut m, 3, 3, |r| {
        r.insert(BODY, Value::Utf8("lazy dog".into()));
    });
...
fn graph_traversal_sees_recent_writes() {
    let mut m = Memtable::new();
    put(&mut m, 1, 1, |r| {
        r.insert(FOLLOWS, Value::Edges(vec![2, 3]));
    });
    put(&mut m, 2, 2, |r| {
        r.insert(FOLLOWS, Value::Edges(vec![3]));
    });
    put(&mut m, 3, 3, |r| {
        r.insert(FOLLOWS, Value::Edges(vec![]));
    });
...
fn scalar_access_respects_snapshot() {
    let mut m = Memtable::new();
    put(&mut m, 1, 1, |r| {
        r.insert(9, Value::I64(100));
    });
    put(&mut m, 1, 5, |r| {
        r.insert(9, Value::I64(200));
    });
