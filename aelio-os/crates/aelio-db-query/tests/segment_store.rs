...
use aelio_db_catalog::ColumnKind;
use aelio_db_engine::Value;
use aelio_db_query::Database;
use aelio_db_storage::{CachedSegmentStore, LocalSegmentStore, MemorySegmentStore, SegmentStore};
...
fn flush_publishes_to_memory_and_reopens_via_download() {
    ...
    let store: Arc<dyn SegmentStore> = Arc::new(CachedSegmentStore::new(
        local,
        Arc::clone(&remote) as Arc<dyn SegmentStore>,
    ));

    {
        let mut db =
            Database::create_with_store(dir.path().to_path_buf(), Arc::clone(&store)).unwrap();
        db.create_table("t", &[("name", ColumnKind::Utf8), ("n", ColumnKind::I64)])
            .unwrap();
        db.insert(
            "t",
            &[("name", Value::Utf8("alpha".into())), ("n", Value::I64(1))],
        )
        .unwrap();
        db.insert(
            "t",
            &[("name", Value::Utf8("beta".into())), ("n", Value::I64(2))],
        )
        .unwrap();
        db.flush().unwrap();
    }
    ...
    assert!(
        remote.contains("seg-00000.vss"),
        "segment must be uploaded to remote"
    );
    ...
    let rows = db
        .scan_values("t", &aelio_db_query::HybridQuery::new(10))
        .unwrap();
...
fn compact_prunes_old_segments_from_remote() {
    ...
    db.insert("t", &[("name", Value::Utf8("one".into()))])
        .unwrap();
    db.flush().unwrap();
    assert!(remote.contains("seg-00000.vss"));

    db.insert("t", &[("name", Value::Utf8("two".into()))])
        .unwrap();
    db.flush().unwrap();
    ...
    assert!(
        !remote.contains("seg-00000.vss"),
        "old segment pruned from cloud"
    );
    assert!(
        !remote.contains("seg-00001.vss"),
        "old segment pruned from cloud"
    );
