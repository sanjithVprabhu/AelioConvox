...
use aelio_db_query::{ColumnKind, Database, HybridQuery, Value};
...
impl TmpDir {
    fn new() -> Self {
        ...
        let p = std::env::temp_dir().join(format!("aelio_rec_{}_{n}", std::process::id()));
...
fn reopen_recovers_segment_rows_and_wal_tail_once() {
    let dir = TmpDir::new();
    {
        ...
        db.insert("docs", &doc([0.0, 0.0], "alpha volatility", 2020))
            .unwrap();
        ...
        db.insert("docs", &doc([5.0, 0.0], "gamma volatility", 2022))
            .unwrap();
    }
    ...
    assert_eq!(
        unique.len(),
        ids.len(),
        "no double-counting across WAL + segment: {ids:?}"
    );
}
...
fn reopen_continues_row_ids_and_flushes_again() {
    let dir = TmpDir::new();
    {
        ...
        assert_eq!(
            db.insert("docs", &doc([1.0, 0.0], "b volatility", 2))
                .unwrap(),
            2
        );
        db.flush().unwrap();
    ...
    let id3 = db
        .insert("docs", &doc([2.0, 0.0], "c volatility", 3))
        .unwrap();
