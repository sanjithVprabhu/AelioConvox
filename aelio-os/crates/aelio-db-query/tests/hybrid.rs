...
impl TmpDir {
    fn new() -> Self {
        ...
        let p = std::env::temp_dir().join(format!("aelio_db_{}_{n}", std::process::id()));
...
fn hybrid_query_spans_memtable_and_segments() {
    ...
    db.insert(
        "documents",
        &doc([0.0, 0.0], "hedging derivatives volatility", vec![], 2020),
    )
    .unwrap();
    db.insert(
        "documents",
        &doc([10.0, 0.0], "cooking recipes", vec![], 2021),
    )
    .unwrap();
    db.insert(
        "documents",
        &doc([5.0, 0.0], "volatility models", vec![], 2022),
    )
    .unwrap();
    ...
    db.insert(
        ...
        &doc(
            [4.0, 0.0],
            "derivatives and volatility hedging",
            vec![],
            2023,
        ),
    )
    .unwrap();
    db.insert("documents", &doc([99.0, 0.0], "gardening", vec![], 2024))
        .unwrap();
    ...
    assert_eq!(
        sorted,
        vec![1, 3, 4],
        "fused top-3 = on-topic rows across both sources"
    );
    assert!(
        ids.contains(&4),
        "a recent (memtable-only) row participates in the ranking"
    );

    // The plan is a single hybrid operator tree over multiple sources.
    let explain = db.explain("documents", &q).unwrap();
    assert!(
        explain.contains("VectorSearch")
            && explain.contains("TextMatch")
            && explain.contains("RRF")
    );
}
...
fn scalar_filter_and_table_isolation() {
    ...
    let mut db = Database::create(&dir.0).unwrap();
    db.create_table("documents", &schema()).unwrap();
    db.create_table("other", &[("embedding", ColumnKind::Vector(2))])
        .unwrap();

    db.insert(
        "documents",
        &doc([1.0, 0.0], "alpha volatility", vec![], 2019),
    )
    .unwrap();
    db.insert(
        "documents",
        &doc([2.0, 0.0], "beta volatility", vec![], 2023),
    )
    .unwrap();
    // A row in a different table with a colliding vector — must NOT appear in documents queries.
    db.insert("other", &[("embedding", Value::Vector(vec![1.0, 0.0]))])
        .unwrap();
    db.flush().unwrap();

    // "volatility" but only year >= 2020 → excludes the 2019 row.
    let q = HybridQuery::new(10).text("body", "volatility").filter(
        "year",
        PredOp::Ge,
        Value::I64(2020),
    );
...
fn graph_constraint_in_hybrid_query() {
    ...
    db.insert("documents", &doc([0.0, 0.0], "topic one", vec![2], 2020))
        .unwrap();
    db.insert("documents", &doc([0.0, 0.0], "topic two", vec![3], 2020))
        .unwrap();
    db.insert("documents", &doc([0.0, 0.0], "topic three", vec![], 2020))
        .unwrap();
    db.insert("documents", &doc([0.0, 0.0], "topic four", vec![], 2020))
        .unwrap();
    db.flush().unwrap();
    ...
    assert_eq!(
        ids,
        vec![2, 3],
        "only docs reachable from row 1 within 2 hops"
    );
}
