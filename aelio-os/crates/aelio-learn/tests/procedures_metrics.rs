...
fn step(id: &str, shape: &str) -> TraceStep {
    TraceStep { call_id: id.into(), arg_shape: shape.into() }
}

fn trace(steps: &[(&str, &str)], success: bool, day: u32) -> FlowTrace {
    FlowTrace { steps: steps.iter().map(|(i, s)| step(i, s)).collect(), success, day }
}

#[test]
fn mines_recurring_successful_subsequence() {
    // validate_phone → send_otp recurs in 6 successful in-window flows.
    let common = &[("compute.validate_phone", "{v:str}"), ("tool.send_otp", "{to:str}")][..];
    ...
    traces.push(trace(&[("compute.validate_phone", "{v:str}"), ("tool.send_otp", "{to:str}")], false, 1));

    let candidates = mine(&traces, &MiningParams::default(), 10);
    assert!(
        candidates.iter().any(|c| c.signature
            == vec![step("compute.validate_phone", "{v:str}"), step("tool.send_otp", "{to:str}")]
            && c.occurrences >= 5),
...
fn rare_or_out_of_window_subsequences_are_not_mined() {
    ...
    let rare: Vec<FlowTrace> = (0..4)
        ...
        .collect();
    assert!(mine(&rare, &MiningParams::default(), 10).is_empty(), "≥5 threshold (§19)");

    // 6 occurrences but all outside the 30-day window.
    let stale: Vec<FlowTrace> = (0..6)
        .map(|_| trace(&[("tool.a", "{}"), ("tool.b", "{}")], true, 0))
        .collect();
    assert!(mine(&stale, &MiningParams::default(), 200).is_empty(), "30-day window (§19)");
}
...
fn arg_shape_isomorphism_distinguishes_procedures() {
    ...
    assert!(
        !cands.iter().any(|c| c.signature.first().map(|s| s.call_id.as_str()) == Some("tool.x")),
...
fn converter_kill_criterion() {
    ...
    let bad = ConverterMetrics { warm_hits: 50, total_lookups: 100, days_observed: 95 };
    assert!(converter_falsified(&bad, &kc));
    // Above the floor ⇒ not falsified.
    let good = ConverterMetrics { warm_hits: 70, total_lookups: 100, days_observed: 95 };
    assert!(!converter_falsified(&good, &kc));
    // Before the window ⇒ not yet judged.
    let early = ConverterMetrics { warm_hits: 10, total_lookups: 100, days_observed: 30 };
...
fn procedure_kill_criterion() {
    ...
    let bad = ProcedureMetrics { turns_touching_a_procedure: 5, total_turns: 100, days_observed: 190, active_tenants: 3 };
    assert!(procedure_demoted_to_experimental(&bad, &kc));
    // Only 2 tenants ⇒ not enough evidence to judge.
    let thin = ProcedureMetrics { active_tenants: 2, ..bad };
