...
fn decision_point_requires_fallback_and_caps_at_eight() {
    let good = DecisionPoint {
        id: "unauth.v1".into(),
        fallback_pathway: "divert".into(),
        hygiene: hyg(),
    };
    assert!(validate_decision_point(&good, 8).is_ok());
    assert!(
        validate_decision_point(&good, MAX_PATHWAYS + 1).is_err(),
        "cap 8 (§18)"
    );
    let no_fb = DecisionPoint {
        fallback_pathway: "".into(),
        ..good
    };
    assert!(
        validate_decision_point(&no_fb, 3).is_err(),
        "fallback mandatory (§18)"
    );
}
...
fn attribution_is_conservative_boolean() {
    ...
    assert_eq!(
        win.iter()
            .find(|c| c.artifact_id == "path.login")
            .unwrap()
            .credit,
        2.0
    );
