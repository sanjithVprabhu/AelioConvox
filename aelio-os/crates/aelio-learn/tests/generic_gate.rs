...
fn converter_pathway_procedure_share_one_gate() {
    ...
    let rules = parse_rules(&serde_json::json!([{"op":"rename","from":"active","to":"loggedin"}])).unwrap();
    let digest = Digest::new([("loggedin", "bool")]);
    let conv_obs: Vec<(String, bool)> = (0..20)
        .map(|i| {
            let input = SolValue::map([("active", SolValue::Bool(i % 2 == 0))]);
            (format!("conv-{i}"), gate::shadow_validate(&rules, &input, &digest))
        })
        .collect();
    assert!(advances(&conv_obs), "correct converter advances through the shared gate");

    // ── Pathway: agreement = retrospective outcome match on incumbent-failed turns (§18). ──
    let pw_obs: Vec<(String, bool)> = (0..30)
        .filter_map(|i| {
            let turn = PathwayTurn { incumbent_failed: i % 3 != 0, new_would_succeed: true };
            pathway_agreement(&turn).map(|agreed| (format!("pw-{i}"), agreed))
        })
        .collect();
    assert!(pw_obs.len() >= 20, "enough incumbent-failed turns to judge");
    assert!(advances(&pw_obs), "a challenger that fixes incumbent failures advances (same gate)");

    // ── Procedure: agreement = shadow-compare predicted vs actual sequence (§19). ──
    let predicted = vec![
        TraceStep { call_id: "compute.validate_phone".into(), arg_shape: "{v:str}".into() },
        TraceStep { call_id: "tool.send_otp".into(), arg_shape: "{to:str}".into() },
    ];
    let proc_obs: Vec<(String, bool)> = (0..20)
        .map(|i| (format!("proc-{i}"), procedure_agreement(&predicted, &predicted)))
        .collect();
    assert!(advances(&proc_obs), "a faithfully-predicted procedure advances (same gate)");

    // A mispredicted procedure disagrees and does not advance.
    let wrong = vec![TraceStep { call_id: "tool.other".into(), arg_shape: "{}".into() }];
    let bad_obs: Vec<(String, bool)> = (0..20)
        .map(|i| (format!("proc-bad-{i}"), procedure_agreement(&predicted, &wrong)))
        .collect();
    assert!(!advances(&bad_obs), "a mispredicting procedure is caught by the same gate");
}
...
fn evidence_cannot_be_inflated_by_repetition() {
    ...
    let repeated: Vec<(String, bool)> = (0..100).map(|i| (format!("input-{}", i % 5), true)).collect();
    assert!(!advances(&repeated), "repetition can't inflate distinct-input evidence (§13.2)");
}
