...
pub enum Selection {
    ...
    Picked { pathway_id: String, top: f64, margin: f64, entropy: f64 },
    /// All hygiene checks are ANDed; any failure routes to the declared fallback.
    Fallback { reason: FallbackReason, entropy: f64 },
}
...
pub fn validate_decision_point(dp: &DecisionPoint, candidate_count: usize) -> Result<(), String> {
    ...
    if candidate_count > MAX_PATHWAYS {
        return Err(format!("decision point exceeds {MAX_PATHWAYS} pathways (§18)"));
    }
    Ok(())
}
...
pub fn select(candidates: &[PathwayScore], hygiene: &Hygiene) -> Selection {
    ...
    if candidates.is_empty() {
        return Selection::Fallback { reason: FallbackReason::Empty, entropy };
    }
    ...
    if top < hygiene.tau {
        return Selection::Fallback { reason: FallbackReason::LowConfidence, entropy };
    }
    if margin < hygiene.delta {
        return Selection::Fallback { reason: FallbackReason::ThinMargin, entropy };
    }
    if entropy > hygiene.entropy_ceiling {
        return Selection::Fallback { reason: FallbackReason::HighEntropy, entropy };
    }
    Selection::Picked { pathway_id: sorted[0].pathway_id.clone(), top, margin, entropy }
}
