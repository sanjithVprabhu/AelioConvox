...
/// another crate. `aelio-db-format` only frames and CRC-verifies these; it does not interpret
/// them.
...
///
/// `extra` carries opaque pre-serialized sections (e.g. an HNSW index from `aelio-db-index`);
