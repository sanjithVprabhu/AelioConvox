...
fn roundtrip_all_scalar_types() {
    ...
    let columns = vec![
        col(
            1,
            false,
            ColumnValues::I64(vec![Some(1), Some(2), Some(3), Some(-7), Some(100)]),
        ),
        col(
            2,
            false,
        ...
        col(
            6,
            false,
            ColumnValues::I32(vec![Some(-1), Some(0), Some(1), Some(2), Some(3)]),
        ),
        col(
            7,
            false,
...
fn roundtrip_with_nulls() {
    ...
    let columns = vec![
        col(
            1,
            false,
            ColumnValues::I64(vec![Some(5), None, Some(7), None]),
        ),
        col(
            2,
            false,
...
fn detects_column_page_corruption_via_section_crc() {
    ...
    let columns = vec![col(
        1,
        false,
        ColumnValues::I64((0..10).map(Some).collect()),
    )];
