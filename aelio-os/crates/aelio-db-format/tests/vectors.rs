...
fn multipage_vectors_have_per_page_centroids() {
    ...
    write_file_with(
        ...
        &ids(n),
        &[],
        &WriteOptions { rows_per_page: 2 },
    )
    .expect("write");
    let file = read_file(&tmp.0).expect("read");

    assert_eq!(
        file.columns[0].values,
        ColumnValues::Vector { dim: 2, data }
    );
    let zms: Vec<_> = file
        .footer
        .zone_maps
        .iter()
        .filter(|z| z.column_id == 3)
        .collect();
