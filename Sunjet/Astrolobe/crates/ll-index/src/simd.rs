...
unsafe fn l2q_avx2(a: &[u8], b: &[u8], scale: &[f32]) -> f32 {
    ...
    while i + 8 <= len {
        ...
        let ai = _mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(_mm_loadl_epi64(a.as_ptr().add(i) as *const _)));
        let bi = _mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(_mm_loadl_epi64(b.as_ptr().add(i) as *const _)));
...
mod tests {
    ...
    fn simd_matches_scalar() {
        for dim in [1usize, 7, 8, 16, 31, 32, 100, 257] {
            ...
            assert!((s - v).abs() <= 1e-3 * (1.0 + s.abs()), "dim {dim}: {s} vs {v}");
        }
    }
