...
use ll_index::{distance, serialize_hnsw, Hnsw, HnswParams, HnswView, Metric, SplitMix64};
...
fn serialized_search_preserves_recall() {
    ...
    let mut index = Hnsw::new(
        dim,
        HnswParams { m: 16, ef_construction: 200, seed: 42, metric: Metric::L2 },
    );
    for v in &data {
    ...
    for q in &queries {
        let truth: std::collections::HashSet<u32> =
            brute_force(q, &data, k).into_iter().collect();
