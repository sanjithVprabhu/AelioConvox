...
//! This closes the storage loop: `ll-index` produces the index bytes, `ll-format` frames
...
fn filtered_search_from_a_reopened_vss_file() {
    ...
    let mut index = Hnsw::new(dim, HnswParams { m: 16, ef_construction: 100, seed: 42, metric });
    for v in &data {
    ...
    let meta = FileMeta {
        ...
        writer_version: "ll-index-e2e".into(),
    };
    ...
    write_file_with(&path.0, &meta, &[vec_col, filt_col], &translation, &extra, &WriteOptions::default())
        .expect("write");
    ...
    let read_vecs: Vec<Vec<f32>> = match &file.columns.iter().find(|c| c.column_id == VEC_COL).unwrap().values {
    ...
    let read_filt: Vec<f32> = match &file.columns.iter().find(|c| c.column_id == FILT_COL).unwrap().values {
    ...
    let section = file.section(SectionType::Hnsw, VEC_COL).expect("hnsw section present");
    ...
    for q in &queries {
        ...
        let hits = res.results.iter().filter(|(id, _)| truth_set.contains(id)).count();
        total_recall += hits as f64 / truth_set.len().max(1) as f64;
    }
    let recall = total_recall / queries.len() as f64;
    assert!(recall >= 0.95, "end-to-end filtered recall {recall:.3} too low");
}
