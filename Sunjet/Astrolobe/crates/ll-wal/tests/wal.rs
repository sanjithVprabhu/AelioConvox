...
use ll_wal::{replay, Record, RecordType, Wal};
...
impl TempPath {
    fn new() -> Self {
        ...
        TempPath(std::env::temp_dir().join(format!("ll_wal_{}_{n}.log", std::process::id())))
    }
}
...
fn append_and_replay_roundtrip() {
    ...
    assert_eq!(
        r.records[0],
        Record { lsn: 100, txn_id: 1, rtype: RecordType::InsertRow, payload: b"row-a".to_vec() }
    );
...
fn replay_stops_at_first_crc_mismatch() {
    ...
    let r = replay(&p.0).unwrap();
    assert_eq!(r.records.len(), 1, "replay must stop at the first corrupt record");
