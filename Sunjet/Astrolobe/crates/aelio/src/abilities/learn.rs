...
pub fn propose_path_prompt_spec() -> PromptSpec {
    PromptSpec {
        ...
        instruction: concat!(
            ...
            "Return exactly one JSON object with a steps array of strings."
        )
        .into(),
        slots: vec![
            PromptSlotSpec {
        ...
        output: ClosedOutputSpec {
            ...
            field_types: indexmap::indexmap! {
                "steps".into() => ClosedOutputType::StringArray,
            },
        },
    }
}
...
pub fn propose_path_with_provider(
    ...
    let bindings = indexmap::indexmap! {
        ...
        "abilities".into() => ability_ids.join("\n"),
    };
    ...
    let steps = parsed
        ...
        .iter()
        .map(|step| {
            step.as_str().map(str::to_string).ok_or_else(|| {
                AelioError::new(
                    ReasonCode::ParseError,
                    "provider output path steps must be strings",
                )
            })
        })
    ...
    if let Some(unknown) = steps
        .iter()
        .find(|step| !ability_ids.iter().any(|ability| ability == *step))
    {
        return Err(AelioError::new(
            ReasonCode::ParseError,
            format!("provider selected undeclared ability `{unknown}`"),
        ));
    }
    Ok((AbilityPath::seq(steps), prompt_hash))
}
