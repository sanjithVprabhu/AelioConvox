...
fn durable_turn_request_hash(request: &DurableTurnRequest) -> String {
    use sha2::{Digest, Sha256};

    let mut hasher = Sha256::new();
    for value in [&request.user_id, &request.utterance] {
...
impl ToolHost for DurableToolHost {
    ...
    fn call_with_context(
        ...
        match self.inner.call(&tool.id, args) {
            Ok(raw) => {
...
/// Owns the tenant world and its embedded Astrolobe repositories.
...
impl DurableRuntime {
    ...
    pub fn run_turn(&mut self, request: DurableTurnRequest) -> AelioResult<TurnResult> {
        ...
        self.refresh_procedures()?;
        let now = Utc::now().timestamp_millis();
        let request_hash = durable_turn_request_hash(&request);
        ...
        let outcome = (|| -> AelioResult<TurnResult> {
            ...
            let mut result = self.world.run_turn(&request.user_id, &request.utterance);
    ...
    match predicate {
        ...
        Predicate::And { of } | Predicate::Or { of } => {
            if of.is_empty() {
                return Err(AelioError::new(
                    ReasonCode::Validation,
                    "and/or policy predicates must not be empty",
                ));
            }
            for child in of {
                validate_closed_predicate(child)?;
            }
            Ok(())
        }
        Predicate::Not { of } => validate_closed_predicate(of),
    }
}
...
fn validate_predicate_path(path: &str) -> AelioResult<()> {
    ...
    let valid = match root {
        "state" | "role" | "tool" | "consent" => child.is_none(),
...
mod tests {
    ...
    fn durable_idempotency_replays_result_after_restart() {
        ...
        };
        {
            ...
            host.call_with_context(&tool, &args, "idem-restart", "user-1")
                .unwrap();
        }
        store.flush().unwrap();
        drop(store);
    ...
    fn unknown_non_idempotent_outcome_requires_manual_review() {
        ...
        let error = runtime
            .world
            .tool_host
            .call_with_context(&tool, &IndexMap::new(), "unknown-effect", "user-1")
            .unwrap_err();
