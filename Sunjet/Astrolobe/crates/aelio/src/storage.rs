//! Durable, tenant-scoped Aelio repositories over the embedded Astrolobe database.
//!
...
//! logical isolation boundaries over Astrolobe's shared WAL and segments; authorization still
...
/// Resolves a tenant and closed logical name to a physical Astrolobe table.
#[derive(Debug, Clone)]
...
impl AelioStore {
    ...
    /// BM25 search with tenant-table, status, and expiry predicates applied in Astrolobe.
    ...
    /// Tenant-scoped graph traversal. Intermediate hops are table-scoped by ll-query.
...
fn query_error(error: ll_query::DatabaseQueryError) -> AelioError {
    match error {
        ll_query::DatabaseQueryError::Execution(ll_query::QueryError::GraphBudgetExceeded(
            exceeded,
        )) => {
            let message = format!(
        ...
        ll_query::DatabaseQueryError::Planning(error) => storage_error(error),
    }
}
