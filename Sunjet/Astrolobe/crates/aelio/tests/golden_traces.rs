...
use aelio::blocks::flow::FlowInstance;
use aelio::runtime::World;
use aelio::types::{Depth, LookupTier};
...
fn flow_effect_is_policy_wrapped() {
    use aelio::contract::Predicate;
    use aelio::tenant::{PolicyAction, PolicyEffect, PolicySpec, PolicySubject};
...
fn arbitrary_capability_path_executes_generically() {
    ...
    world.registry.register_ability(
        aelio::AbilityContract::effect("custom.arbitrary.ping")
            .with_tool_deps(vec!["custom_ping".into()])
            .with_postconditions(vec![aelio::Predicate::Present {
                path: "evidence.pong".into(),
            }]),
    );
    world.set_tool_host(Box::new(
        aelio::abilities::invoke::MockToolHost::default().on("custom_ping", |_| {
            Ok(aelio::Value::Map(indexmap::indexmap! {
                "pong".into() => aelio::Value::Bool(true),
            }))
        }),
    ));
    ...
    match result {
        PathExecution::Done {
            ...
            assert_eq!(frame.evidence.get("pong"), Some(&aelio::Value::Bool(true)));
        }
...
fn composed_tools_pass_only_declared_sanitized_evidence_to_the_next_step() {
    ...
    use aelio::contract::AbilityPath;
    use aelio::policy::PolicyCtx;
    use aelio::tenant::{OutputField, OutputSpec, ParamSource, ParamSpec, ToolSpec};
    use aelio::types::Sensitivity;
    ...
    world.set_tool_host(Box::new(
        aelio::abilities::invoke::MockToolHost::default()
            .on("resolve_customer", |_| {
                Ok(aelio::Value::Map(indexmap::indexmap! {
                    "customer_id".into() => aelio::Value::str("customer-7"),
                }))
            })
            .on("list_invoices", |args| {
                if args.get("customer_id").and_then(aelio::Value::as_str) != Some("customer-7") {
                    return Err(aelio::AelioError::new(
                        aelio::ReasonCode::Missing,
                        "customer_id was not handed off",
                    ));
                }
                Ok(aelio::Value::Map(indexmap::indexmap! {
                    "invoice_list".into() => aelio::Value::List(vec![
                        aelio::Value::str("inv-1"),
                        aelio::Value::str("inv-2"),
                    ]),
                }))
            }),
    ));
    ...
    match result {
        PathExecution::Done {
            ...
            assert!(matches!(
                ...
                Some(aelio::Value::List(values)) if values.len() == 2
            ));
        }
...
impl aelio::embedding::Embedder for CrossLanguageQueryEmbedder {
    fn dimension(&self) -> usize {
        2
    }

    fn embed(&self, text: &str) -> aelio::AelioResult<Vec<f32>> {
...
fn non_english_rank_request_uses_closed_model_extraction_then_declared_execution() {
    ...
    world.set_llm_provider(Box::new(
        aelio::provider::ScriptedLlmProvider::deterministic().with_fallback(
...
fn tier_zero_deep_path_uses_generic_executor() {
    use aelio::abilities::learn::{situation_hash, situation_key};
    use aelio::abilities::registry::{
        ProcedureEvidence, ProcedureProvenance, ProcedureSpec, ProcedureStatus, SituationFilter,
    };
    use aelio::contract::{AbilityContract, AbilityPath, Predicate};
    use aelio::tenant::{OutputSpec, ToolSpec};
    ...
    world.registry.register_ability(
        AbilityContract::effect("telemetry.snapshot.read")
            .idempotent()
    ...
    world.set_tool_host(Box::new(
        aelio::abilities::invoke::MockToolHost::default().on("read_snapshot", |_| {
            Ok(aelio::Value::Map(indexmap::indexmap! {
                "snapshot".into() => aelio::Value::str("healthy"),
            }))
        }),
    ));
...
fn persisted_term_confirmation_executes_the_exact_declared_plan() {
    ...
    world.user_flows.insert(
        "u1".into(),
        aelio::blocks::flow::FlowInstance {
...
fn multi_clause_not_swallowed_as_hi() {
    let (skipped, clauses) = aelio::abilities::understand::split_clauses(
    ...
    let depth = aelio::abilities::understand::classify_depth(
        "Hi, I need to cancel my order and also refund",
    );
...
fn binder_only_residual_phone() {
    use aelio::abilities::bind::{resolve_all, BindSources};
    use aelio::types::Value;
...
fn policy_denies_when_configured() {
    use aelio::contract::Predicate;
    use aelio::policy::{evaluate, PolicyCtx, PolicyDecision};
    use aelio::tenant::{PolicyAction, PolicyEffect, PolicySpec, PolicySubject};
...
fn tool_invalidation_demotes_procedures() {
    use aelio::abilities::learn::{propose_path_greeting, situation_hash, situation_key};
    use aelio::abilities::registry::{
        ProcedureEvidence, ProcedureProvenance, ProcedureSpec, ProcedureStatus, SituationFilter,
    };
    use aelio::contract::AbilityContract;
...
fn sig_mismatch_does_not_guess() {
    use aelio::abilities::sig::{self, ExtractionPlan, PlanStatus, SignatureRegistry};
    use aelio::types::Value;
...
fn login_flow_not_learnable() {
    ...
    assert!(!aelio::blocks::flow::may_reorder(flow));
}

/// Tier-2 composition: customer_id → invoices.
#[test]
fn tier2_composes_procedures() {
    use aelio::abilities::learn::{compose, typecheck};
    use aelio::abilities::registry::{
        ProcedureEvidence, ProcedureProvenance, ProcedureSpec, ProcedureStatus, SituationFilter,
    };
    use aelio::contract::{AbilityContract, AbilityPath, Predicate};
    ...
    let path = compose(
        &world.registry,
        &aelio::abilities::learn::GoalSpec {
        ...
        aelio::abilities::learn::CompositionBudget::default(),
    )
