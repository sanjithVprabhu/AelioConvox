...
use aelio::storage::{AelioStore, CompareSwap, LogicalTable, StoredRecord};
use aelio::tenant::{PolicyAction, PolicyEffect, PolicySpec, PolicySubject};
use aelio::ReasonCode;
...
fn live_recall_enforces_hit_and_character_budgets() {
    ...
    let chars: usize = evidence
        ...
        .iter()
        .map(|claim| {
            aelio::ops::pure::value_to_json(&claim.value)
                .to_string()
                .chars()
                .count()
        })
...
fn concurrent_proactive_candidates_cannot_exceed_the_daily_cap() {
    ...
    assert!(outcomes
        ...
        .any(|error| error.code == ReasonCode::Conflict));
    let jobs: Vec<StoredRecord<ScheduledJob>> = shared
        .list("tenant-1", LogicalTable::Jobs, None, 10)
        .unwrap();
    assert_eq!(jobs.len(), 1);
