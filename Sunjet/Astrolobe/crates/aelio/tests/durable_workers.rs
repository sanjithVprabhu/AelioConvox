...
use aelio::storage::{AelioStore, LogicalTable, RecordEnvelope, StoredRecord};
use aelio::tenant::{FlowActivation, FlowEscape, FlowSpec, FlowStep, Preemption, ViolationAction};
use aelio::types::{LookupTier, TenantMode};
use aelio::{Predicate, ReasonCode};
...
fn job(id: &str) -> ScheduledJob {
    ScheduledJob {
        ...
        state: aelio::runtime::JobState::Scheduled,
        scheduled_at_ms: 10,
        owner: None,
...
fn proposal(id: &str, effectful: bool, evidence: ProposalEvidence) -> ColdProposal {
    ColdProposal {
        ...
        contract: aelio::contract::AbilityContract::pure(format!("contract-{id}")),
        effectful,
...
fn crash_reconciliation_never_repeats_unknown_non_idempotent_effect() {
    ...
    assert!(outcomes.iter().any(|outcome| matches!(
        outcome,
        aelio::runtime::ReconcileOutcome::NonIdempotentManualReview { .. }
    )));
    assert!(outcomes.iter().any(|outcome| matches!(
        outcome,
        aelio::runtime::ReconcileOutcome::TurnManualReview { .. }
    )));
...
fn promotion_race_creates_one_immutable_version() {
    ...
    let procedures: Vec<StoredRecord<aelio::runtime::PromotedProcedureVersion>> = shared
...
fn promoted_procedure_can_learn_a_ranked_immutable_v2() {
    ...
    let versions: Vec<StoredRecord<aelio::runtime::PromotedProcedureVersion>> = shared
        .list("tenant-1", LogicalTable::Procedures, Some("promoted"), 10)
        .unwrap();
    ...
    let runtime = DurableRuntime::new(aelio::World::demo_tenant("tenant-1"), shared).unwrap();
    assert_eq!(
...
fn promoted_procedure_is_tier_zero_after_database_restart() {
    ...
    let world = aelio::World::demo_tenant("tenant-1");
    let sigma = aelio::abilities::learn::situation_key(
        "unauthenticated",
        "greeting",
        vec![],
    ...
    durable_proposal.situation_hash = aelio::abilities::learn::situation_hash(&sigma);
    ...
    let mut runtime =
        aelio::runtime::DurableRuntime::new(aelio::World::demo_tenant("tenant-1"), reopened)
            .unwrap();
    let result = runtime
        .run_turn(aelio::runtime::DurableTurnRequest {
    ...
    let loaded = runtime
        .world
        .registry
        .lookup_procedure_by_hash(&aelio::abilities::learn::situation_hash(&sigma))
        .unwrap();
...
fn dependency_changes_invalidate_without_mutating_promoted_version() {
    ...
    let mut prompt_v1_registry = aelio::abilities::registry::Registry::default();
    let mut prompt_v1 = aelio::AbilityContract::pure("Express.Template");
    ...
    let stored: StoredRecord<aelio::runtime::PromotedProcedureVersion> = shared
    ...
    prompt_dependent.dependencies.prompt_hash =
        aelio::abilities::learn::path_prompt_dependency_fingerprint(
            &prompt_v1_registry,
    ...
    let mut prompt_v2_registry = aelio::abilities::registry::Registry::default();
    let mut prompt_v2 = aelio::AbilityContract::pure("Express.Template");
...
fn repeated_success_promotes_durably_and_next_turn_is_tier_zero() {
    ...
    let mut runtime =
        DurableRuntime::new(aelio::World::demo_tenant("tenant-1"), shared.clone()).unwrap();
    for index in 0..5 {
        let result = runtime
...
fn normalized_rank_query_learns_through_the_same_tier_ladder() {
    ...
    let mut world = aelio::World::demo_tenant("tenant-1");
...
fn distinct_term_confirmations_promote_a_durable_tenant_synonym() {
    ...
    drop(runtime);
    let restarted = DurableRuntime::new(aelio::World::demo_tenant("tenant-1"), shared).unwrap();
...
fn live_shadow_exploration_is_read_only_bounded_and_restart_durable() {
    ...
    let situation_hash = aelio::abilities::learn::situation_hash(&sigma);
    promote_source(
        shared.clone(),
        "primary-greeting",
        &situation_hash,
    ...
    let comparisons: Vec<StoredRecord<aelio::runtime::ExplorationComparison>> = shared
...
impl aelio::embedding::Embedder for ConstantSemanticEmbedder {
    fn dimension(&self) -> usize {
        3
    }

    fn embed(&self, _text: &str) -> aelio::AelioResult<Vec<f32>> {
        Ok(vec![0.0, 1.0, 0.0])
    }
...
fn durable_promotion_and_tier_one_use_the_same_configured_embedding_space() {
    ...
    let embedder: Arc<dyn aelio::embedding::Embedder> = Arc::new(ConstantSemanticEmbedder("v1"));
    let mut runtime = DurableRuntime::new_with_embedder(
        aelio::World::demo_tenant("tenant-1"),
        shared.clone(),
        embedder,
    )
    .unwrap();
