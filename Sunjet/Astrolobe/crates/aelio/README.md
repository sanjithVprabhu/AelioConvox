...
## Relationship to Sunjet

Sunjet (`ll-*` crates) is the multimodal storage spine. This crate is the **harness
decision engine** that will sit on top of Sunjet for durable procedures, signatures,
proposals, and per-tenant tables. Storage integration is the next milestone; the
typed runtime and turn spine are self-contained and fully tested in-memory today.
