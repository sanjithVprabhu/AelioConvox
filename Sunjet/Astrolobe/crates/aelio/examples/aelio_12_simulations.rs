...
use aelio::embedding::HashEmbedder;
use aelio::memory::{DecayPolicy, Memory, MemoryKind, MemoryProvenance, MemoryStore, ValidTime};
use aelio::policy::{evaluate, explain, PolicyCtx};
use aelio::runtime::{
    DurableRuntime, DurableTurnRequest, ProactiveCandidate, ProactiveLoop, ProactivePolicy,
};
use aelio::storage::{AelioStore, LogicalTable};
use aelio::tenant::{
    OutputField, OutputSpec, ParamSource, ParamSpec, PolicyAction, PolicyEffect, PolicySpec,
    PolicySubject, ToolSpec,
};
use aelio::types::{AelioResult, Sensitivity, Value};
use aelio::World;
...
fn fixture(tag: &str, configure: impl FnOnce(&mut World)) -> Fixture {
    ...
    configure(&mut world);
    let store = AelioStore::new(Database::create(&path).expect("create Astrolobe"), 8)
...
fn value_json(value: &Value) -> Json {
    aelio::ops::pure::value_to_json(value)
}
...
fn safe_raw(tool: &ToolSpec, raw: &Value) -> Json {
    ...
    for field in tool.output_semantics.fields.values() {
        if !matches!(field.sensitivity, Sensitivity::None) {
            value = aelio::path::set_path(&value, &field.path, Value::str("[REDACTED]"))
    ...
    Json::Object(
        tool.output_semantics
            ...
            .filter_map(|(name, field)| {
                let value = aelio::path::get_path(raw, &field.path).ok().flatten()?;
...
fn tool_observations(
    ...
    records
        ...
        .map(|record| {
            ...
            let args_hash = aelio::ops::pure::idem_key(&refs);
            match &record.raw {
                Ok(raw) => json!({
                    ...
                    "raw_signature_hash": aelio::abilities::sig::hash(&aelio::abilities::sig::compute(raw)),
                    "response_role": format!("{:?}", aelio::abilities::sig::classify(raw, tool.output_semantics.role_hint.as_deref())).to_lowercase(),
...
fn policy_observations(
    runtime: &DurableRuntime,
    result: &aelio::blocks::turn::TurnResult,
    state: &str,
) -> Vec<Json> {
    result
        .steps
...
fn flow_json(result: &aelio::blocks::turn::TurnResult) -> Json {
...
fn run_message(
    ...
    message: &str,
    expected: &str,
    assertion: impl FnOnce(&aelio::blocks::turn::TurnResult) -> bool,
) -> Json {
...
fn tier_reason(tier: Option<aelio::LookupTier>) -> &'static str {
    match tier {
        Some(aelio::LookupTier::Tier0) => "exact promoted situation-hash match",
        Some(aelio::LookupTier::Tier1) => "scored vector kNN near-hit (margin ≥ 0.8)",
        Some(aelio::LookupTier::Tier2) => "deterministic procedure composition",
        Some(aelio::LookupTier::Tier3) => "no reusable path; scripted provider proposed one",
        None => "flow, boundary, clarification, or component path bypassed tier lookup",
    }
}

fn summarize(result: &aelio::blocks::turn::TurnResult) -> Json {
    json!({
        "depth": result.depth,
        "tier": result.tier,
...
fn catalog_context(runtime: &DurableRuntime) -> Json {
    json!({
        ...
        "storage": "temporary embedded Astrolobe",
...
fn runtime_scenarios() -> Vec<Json> {
    ...
    let message = run_message(
        ...
        |r| r.tier == Some(aelio::LookupTier::Tier0) && r.llm_calls == 0,
    );
    ...
    let message = run_message(
        ...
        |r| r.tier == Some(aelio::LookupTier::Tier2) && r.llm_calls == 0 && r.proposal_id.is_some(),
    );
    scenarios.push(scenario(
        2,
    ...
    let message = run_message(
        ...
        |r| r.depth == aelio::Depth::Boundary && r.llm_calls == 1,
    );
    scenarios.push(scenario(
        3,
        "Boundary synthesis",
    ...
    let message = run_message(
        ...
        |r| {
            r.tier == Some(aelio::LookupTier::Tier2)
                && r.steps
                    .iter()
    ...
    drop(f.runtime);
    let reopened =
        AelioStore::new(Database::open(&path).expect("reopen Astrolobe"), 8).expect("reopen store");
    let records = Arc::new(Mutex::new(Vec::new()));
    let mut world = World::demo_tenant(TENANT);
    world.set_tool_host(Box::new(RecordingHost::demo(Arc::clone(&records))));
...
This document is generated by the Rust observation harness from actual embedded-Astrolobe runs. \
...
- Runtime: Rust `aelio` crate over temporary embedded Astrolobe databases.\n\
