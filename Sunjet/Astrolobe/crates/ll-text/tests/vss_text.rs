...
fn bm25_from_a_reopened_vss_file() {
    ...
    let meta = FileMeta {
        ...
        writer_version: "ll-text-e2e".into(),
    };
    ...
    write_file_with(&path.0, &meta, &[body_col], &translation, &extra, &WriteOptions::default())
        .expect("write");

    // Reopen and query using only what is in the file.
    let file = read_file(&path.0).expect("read");
    let section = file.section(SectionType::Text, BODY_COL).expect("text section present");
