...
//! transparency.
//!
//! Scope today: full-text (`text`), scalar `filters`, and `graph` reachability. Semantic
...
fn clause_hint(kind: ColumnKind, semantic_enabled: bool) -> &'static str {
    match kind {
        ColumnKind::Text => "full-text search via the `text` clause (BM25)",
        ...
        ColumnKind::Vector(_) => "semantic vector search — NOT available from natural language; ignore for queries",
...
pub fn query_tool_schema(semantic_enabled: bool) -> Value {
    let mut props = json!({
        ...
        "text": {
            "type": "object",
            "description": "BM25 full-text search on a text column",
            "properties": {
                "col": { "type": "string" },
                "query": { "type": "string" }
            },
