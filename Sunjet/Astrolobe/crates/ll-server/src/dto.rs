...
//!
//! These DTOs are deliberately separate from `ll-query`'s internal types: they give the API a
...
use ll_query::{ColumnKind, HybridQuery, PredOp, Value};
    ...
    pub fn to_kind(&self) -> Result<ColumnKind, String> {
        Ok(match self.kind.as_str() {
            ...
            "vector" => {
                let dim = self
                    .dim
                    .ok_or_else(|| format!("column '{}': vector kind requires a 'dim'", self.name))?;
