...
use ll_query::{execute, explain_plan, PredOp, Query, Source};
...
fn selective_filter_triggers_prefilter_and_exact_recall() {
    ...
    let ids: Vec<u64> = execute(sources, &q, snap).into_iter().map(|(id, _)| id).collect();
    assert_eq!(ids, vec![0, 40, 80], "pre-filter must return the exact nearest matches");
