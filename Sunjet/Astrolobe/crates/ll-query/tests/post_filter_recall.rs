...
use ll_index::{serialize_hnsw, Hnsw, HnswParams};
use ll_query::{execute, FileSource, PredOp, Query, Source, Value};
...
fn post_filter_keeps_high_recall_at_moderate_selectivity() {
    ...
    let data: Vec<Vec<f32>> = (0..N).map(|_| (0..DIM).map(|_| rng.f()).collect()).collect();
    ...
    let columns = vec![
        Column {
            column_id: VEC,
            is_system: false,
            values: ColumnValues::Vector { dim: DIM as u16, data: data.iter().map(|v| Some(v.clone())).collect() },
        },
        Column { column_id: CAT, is_system: false, values: ColumnValues::I64(cats.iter().map(|&c| Some(c)).collect()) },
        Column { column_id: SYS_XMIN_COL, is_system: true, values: ColumnValues::I64(vec![Some(1); N]) },
    ];
    let meta = FileMeta {
        file_uuid: [0; 16], min_lsn: 1, max_lsn: 1, row_count: N as u64, schema_fingerprint: 0,
        creation_unix_nanos: 0, schema_blob: Vec::new(),
        mvcc: MvccSummary { min_xmin: 1, max_xmax: u64::MAX, tombstone_count: 0, live_count: N as u64 },
        writer_version: "t".into(),
    };
    let extra = [RawSection { kind: SectionType::Hnsw, column_id: VEC, encoding: SectionEncoding::Plain, bytes: hnsw_bytes }];
    let path = std::env::temp_dir().join(format!("ll_pf_{}.vss", std::process::id()));
    let tt: Vec<u64> = (0..N as u64).collect();
    write_file_with(&path, &meta, &columns, &tt, &extra, &WriteOptions::default()).unwrap();
    ...
    let queries: Vec<Vec<f32>> = (0..20).map(|_| (0..DIM).map(|_| rng.f()).collect()).collect();
    for qv in &queries {
    ...
    assert!(recall >= 0.95, "post-filter recall at ~0.3 selectivity was {recall:.3}");
}
