...
use ll_query::{FileSource, Source};
...
impl Tmp {
    fn new(ext: &str) -> Self {
        ...
        Tmp(std::env::temp_dir().join(format!("ll_fs_{}_{n}.{ext}", std::process::id())))
    }
}
impl Drop for Tmp {
    fn drop(&mut self) {
...
fn query_a_flushed_file_across_all_modalities() {
    ...
    let (l1, l4);
    {
        ...
        l1 = e.insert(1, row([0.0, 0.0], "alpha quick", vec![2, 3], 100)).unwrap();
        e.insert(2, row([10.0, 0.0], "quick quick beta", vec![3], 200)).unwrap();
        e.insert(3, row([5.0, 0.0], "gamma", vec![], 300)).unwrap();
        l4 = e.insert(4, row([3.0, 0.0], "quick delta", vec![1], 400)).unwrap();
    ...
    assert!(!v_old.iter().any(|(id, _)| *id == 4), "row 4 must be invisible before its write");
}
