...
use ll_engine::{Value, SYS_XMAX_COL, SYS_XMIN_COL};
use ll_format::{ColumnValues, LlFile, SectionType};
use ll_graph::EdgeView;
use ll_index::HnswView;
use ll_text::TextView;
...
impl FileSource {
    pub fn new(file: Arc<LlFile>) -> Self {
        ...
        let xmin = match file
            ...
            .map(|c| &c.values)
        {
            Some(ColumnValues::I64(v)) => v.iter().map(|x| x.map(|i| i as u64).unwrap_or(0)).collect(),
            _ => vec![0u64; n],
        };
        let xmax = match file
            .columns
            ...
            .map(|c| &c.values)
        {
            Some(ColumnValues::I64(v)) => v.iter().map(|x| x.map(|i| i as u64).unwrap_or(u64::MAX)).collect(),
            _ => vec![u64::MAX; n],
        };
...
impl Source for FileSource {
    ...
    fn ann_recall_curve(&self, col: u32) -> Vec<(usize, f64)> {
        match self.file.section(SectionType::OptimizerStats, col) {
            Some(bytes) => ll_index::decode_recall_curve(bytes),
            None => Vec::new(),
        }
    }
}
...
/// Convert one cell of an `ll-format` column to an `ll-engine` value.
