...
//!
//! Run: `cargo run --release -p ll-query --example query_bench`
//!
...
//! selectivity by an exact scan (a stored per-column histogram — built & validated in
//! `ll-cost` — removes that scan in production), so OURS latency includes a full scan.
...
fn main() {
    ...
    let path = std::env::temp_dir().join(format!("ll_qbench_{}.vss", std::process::id()));
    write_file_with(&path, &meta, &columns, &local_offsets.iter().map(|&x| x as u64).collect::<Vec<_>>(), &extra, &WriteOptions::default()).unwrap();
    let file = read_file(&path).unwrap();
    let fs = FileSource::new(std::sync::Arc::new(file));
    let sources: &[&dyn Source] = &[&fs];

    let queries: Vec<Vec<f32>> = (0..QUERIES).map(|_| (0..DIM).map(|_| rng.f()).collect()).collect();
    ...
    for &sel in &sels {
        ...
        let q_template = |qv: &[f32]| {
            Query::new(K).with_vector(VEC, qv.to_vec()).filter(CAT, PredOp::Lt, Value::I64(thr))
        };
        let strategy = if explain_plan(sources, &q_template(&queries[0]), SNAP).contains("PreFilter") {
            "PreFilter"
        } else {
            "PostFilter"
        };
