...
fn traversal_from_a_reopened_vss_file() {
    ...
    let meta = FileMeta {
        ...
        writer_version: "ll-graph-e2e".into(),
    };
    ...
    write_file_with(&path.0, &meta, &[id_col], &translation, &extra, &WriteOptions::default())
        .expect("write");

    // Reopen and traverse using only what is in the file.
    let file = read_file(&path.0).expect("read");
    let section = file.section(SectionType::Edge, EDGE_COL).expect("edge section present");
    ...
    assert!(reach2.contains(&1) && reach2.contains(&3) && reach2.contains(&2) && reach2.contains(&4));
}
