...
pub(crate) fn decode_column_chunk(bytes: &[u8]) -> Result<Column> {
    ...
    for p in 0..num_pages {
        ...
        segments.push(decode_page_payload(payload, rows, nullable, lt, vector_dim)?);
    }
...
fn decode_page_payload(
    ...
    let w = lt.fixed_width().expect("non-utf8/non-vector type has fixed width");
    let vals = take(buf, pos, w * m)?;
    macro_rules! build {
    ...
    Ok(match lt {
        ...
        LogicalType::I32 => build!(I32, i32, |c: &[u8]| i32::from_le_bytes(c.try_into().unwrap())),
        LogicalType::I64 => build!(I64, i64, |c: &[u8]| i64::from_le_bytes(c.try_into().unwrap())),
        LogicalType::F32 => build!(F32, f32, |c: &[u8]| f32::from_le_bytes(c.try_into().unwrap())),
        LogicalType::F64 => build!(F64, f64, |c: &[u8]| f64::from_le_bytes(c.try_into().unwrap())),
        LogicalType::TimestampNanos => {
            build!(TimestampNanos, i64, |c: &[u8]| i64::from_le_bytes(c.try_into().unwrap()))
        }
...
fn minmax(col: &ColumnValues, s: usize, e: usize) -> (Vec<u8>, Vec<u8>) {
    ...
    match col {
        ...
        ColumnValues::I32(v) => {
            let (lo, hi) = ord_minmax!(v, s..e);
            bytes_pair(lo.map(|x| x.to_le_bytes().to_vec()), hi.map(|x| x.to_le_bytes().to_vec()))
        }
        ColumnValues::I64(v) | ColumnValues::TimestampNanos(v) => {
            let (lo, hi) = ord_minmax!(v, s..e);
            bytes_pair(lo.map(|x| x.to_le_bytes().to_vec()), hi.map(|x| x.to_le_bytes().to_vec()))
...
mod tests {
    ...
    fn chunk_page_crc_detects_corruption() {
        ...
        assert!(matches!(err, FormatError::PageCrcMismatch { .. }), "{err:?}");
    }
