...
fn decode_value(p: &[u8], pos: usize) -> Option<(Value, usize)> {
    ...
    Some(match tag {
        ...
        T_I64 => (Value::I64(i64::from_le_bytes(p.get(pos..pos + 8)?.try_into().unwrap())), pos + 8),
        T_F64 => (Value::F64(f64::from_le_bytes(p.get(pos..pos + 8)?.try_into().unwrap())), pos + 8),
        T_UTF8 => {
...
fn rd_u32(p: &[u8], o: usize) -> Option<u32> {
    p.get(o..o + 4).map(|s| u32::from_le_bytes(s.try_into().unwrap()))
}
fn rd_u64(p: &[u8], o: usize) -> Option<u64> {
    p.get(o..o + 8).map(|s| u64::from_le_bytes(s.try_into().unwrap()))
}
